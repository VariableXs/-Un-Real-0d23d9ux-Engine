...
**kernel/varix/src/h2star/**（AI-H2 泳道三 H 域二分队，主册《Varix STAR I
start.md》H 域 F251-F300 判据实装层）：50 项功能模块 + h2base 共享底盘
（文件名语义/副本递增/天窗边界/网格落位）+ mod.rs 域聚合器（51 块），
**纯功能代码 10,322 行 · 域内 105 项测试全绿 · 全仓回归 4040+1+6 项零破
坏（隔离验证口径）**。行数对账（实收 20.4% vs 上限口径）与 15 条缺陷账
（真缺陷 9/机械 6，全即时修）如实入 `_attic/aih2-f251-f300/`；验证采用
隔离副本法（工作区含多分队在途包，互不干扰）。

- **F251-F260 交互底盘**：媒体会话仲裁（六态、OSD 归属、<50ms）/ 任务栏
  合并分组（三档、3/7/10 角标）/ 快速访问（5+5 配额共存）/ 透明材质白
  名单（省电降级、WCAG 4.5:1 双向用例）/ 文本拖放四落点（终端停一停）/
  横向滚动（双轴独立到位、冻结列）/ 打开方式（评级排序、不勾不写）/
  桌面右键六项（扩展注入全拦留痕）/ 新建菜单（初态三判据）/ 行内重命名
  （扩展名隔离、非法字符 8 字符抖动拒绝、全选替换语义）。
- **F261-F280 文件与窗口**：删除两路（确认框焦点恒取消）/ 拖拽六组合
  （S: 与 vx: 跨盘判定）/ 发送到（蓝牙动态、防重名阶梯）/ 属性对话框
  （同源双形制、勾选即生效）/ 地址栏补全（末段匹配、真实枚举）/ 导航栈
  （岔路清空、上限 100）/ 存储感知（29/30/31 边界、下载只提醒不动手）/
  磁盘预警三级（每日节流、目录聚合）/ 复制暂停续传（块边界、断点校验）/
  错误重试（人话原因、三选一）/ 多标签（五组键位、拖出拖回）/ 文本框
  右键（置灰不跳位）/ 文档位置记忆（内容键、损坏容错）/ 所有应用（拼音
  二序列）/ 电源菜单（两路同账、长按文档化）/ 贴靠布局（8px 缝、记忆三
  款）/ 多屏任务栏（三策略、按钮随迁）/ 投影模式（指纹记忆）/ 触控板七
  手势（30 次 0 误触）/ 触屏长按（500ms、44px 触区）。
- **F281-F300 通知·系统·账本**：通知细则（两按钮上限、30s 合并、去重）/
  单例三策略（两入口同路由）/ 启动三拍子（2s 门槛诚实等待）/ 无响应恢复
  （抢救快照先存后杀）/ 图标缓存三层（损坏自愈、LRU）/ 壁纸多屏（轮换默
  认关、源删回退）/ 附加时钟（上限 2、离线正确）/ 字体管理（损坏拒绝、
  系统锁定）/ 打印队列（状态机、卡 60s 人话提醒）/ 外设页（离线灰显不消
  失）/ 耗电排行（3 倍异常、只展示不越权）/ 快捷方式健康（三类病、循环
  检测）/ 接入询问（Autorun 零执行路径）/ 安全弹出（强拔标注可发现）/
  时间同步（NTP 2s 阈值、48h 漂移补偿）/ 区域格式（单点服务、KB=1024B）/
  壁纸压暗（30%/15%、浅色直通）/ 磁贴编辑（默认 8、候选 16）/ 推荐区
  （广告位=0 结构保证）/ 图标语汇账本（24px 栅格、孤例=0）。
...
速/焦点环/几何与预算口径）+ mod.rs 域聚合，**wc -l 11,265 行（纯功能面
口径见对账账本）· 域自检 163 条 · 单元测试 93 项，隔离舱 111/111 全绿、
deskstar 面零警告**。

- **F076 快速设置**：六卡聚合 + Wi-Fi 三态 + 飞行聚合射频 + 全键盘 Tab
  循环 + 弹出 ≤100ms 账。
- **F077 通知中心**：toast 5s 沉历史 + 风暴合并 + 免打扰时段跨零点 +
  高优穿透 + 权限硬闸 + 撤销 5s + 相对时间直调 F072（一处一事实）。
- **F078 日历飞出**：前溯格里高利历纯算法，12 月网格全对含 2028 闰二月。
- **F079 全局音效**：六事件 + 静音总闸 100% + 80ms 排队 + -18LUFS 归一。
- **F080 窗口吸附**：八落点无缝 + 过冲 105%→100%（F124 同参）+ 键鼠
  双路一致 + 三分屏驻留推荐。
- **F081 任务视图**：虚拟桌面 + 跨桌焦点跟随 + 关闭并入撤销 + 全键盘。
- **F082 Alt+Tab**：快速/停留双态 + 蛇形十二窗不乱 + 假死灰显仍可选。
- **F083 桌面刷新**：诚实重载 + 80ms 微闪 + 300ms 合并 + 增量 <5ms +
  诚实计数。
- **F084 图标网格**：双模式 + 24px 吸附 + 波浪换位 + 框选 20 刚体。
- **F085 回收站**：零真删红线哈希对拍 100 轮 + 已满三选 + 撤销 5s。
- **F086 复制进度**：3 并行队列 + UI 节流隔离（40GB 不卡）+ 盘满三选。
- **F087 冲突面板**：后缀引擎 100 例 + 应用全部二次确认 + 操作级 undo。
- **F088 搜索即输即显**：防抖零扫描 + 高亮混排 100% + F071 声明式拼音档。
- **F089 标签页资源管理器**：重启全恢复 + 历史栈 20 步引擎（F090 共用）。
- **F090 面包屑**：兄弟下拉 + 投放移动 + 补全 <50ms + 零残留。
- **F091 详情窗格**：真 EXIF 解析器（TIFF IFD 五机型全对）+ 有效分辨率
  旋转修正 + 诚实留白。
- **F092 压缩/解压**：真 ZIP 实体——CRC32 + 完整 inflate（stored/fixed/
  dynamic）+ deflate 三档 + ZipCrypto 只读解密 + 路径穿越拦截（F176）+
  损坏抢救 N/M + round-trip 50 例哈希一致。

隔离舱工艺（#[path] 直挂真实文件）随批归档 `_attic/aid1-f076-f092/`；
行数对账（14 项低于主册上限 90%，如实登记留 UI 接线批回炉）与 12 处
实现侧真缺陷账见同目录账本。
