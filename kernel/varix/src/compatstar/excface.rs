...
pub fn run_excface_checks() -> CheckSet {
    CheckSet::merge(run_excface_base_checks(), CheckSet::merge(run_excface_deep_checks(), CheckSet::merge(run_excface_deep2_checks(), CheckSet::merge(run_excface_deep3_checks(), CheckSet::merge(run_excface_deep4_checks(), CheckSet::merge(run_excface_deep5_checks(), CheckSet::merge(run_excface_deep6_checks(), CheckSet::merge(run_excface_deep7_checks(), run_excface_deep8_checks()))))))))
}
...
pub fn run_excface_deep3_checks() -> CheckSet {
    ...
    // 1) dump 形态压缩：512 字节缓冲、稀疏标记（8 个孤立字节 + 长零段——
    //    栈回溯高位零段的真实形态）→ 显著缩小；round-trip 逐字节一致。
    let mut dump = alloc::vec![0u8; 512];
    for i in 0..8usize {
        dump[64 * i] = 0x10 + i as u8; // 标记位 0/64/128/.../448（越界安全）
    }
...
pub fn run_excface_deep6_checks() -> CheckSet {
    ...
    let mut b1 = [0u8; 32];
    let n1 = dump_size_label(512, &mut b1);
    let t1 = core::str::from_utf8(&b1[..n1]).unwrap_or("");
    let mut b2 = [0u8; 32];
    let n2 = dump_size_label(2048, &mut b2);
    let t2 = core::str::from_utf8(&b2[..n2]).unwrap_or("");
    let mut b3 = [0u8; 32];
    let n3 = dump_size_label(3 * 1024 * 1024, &mut b3);
    let t3 = core::str::from_utf8(&b3[..n3]).unwrap_or("");
    cs.add(
    ...
    let mut b4 = [0u8; 32];
    let n4 = dump_size_label(1536, &mut b4);
    let t4 = core::str::from_utf8(&b4[..n4]).unwrap_or("");
    let mb_half = 1024 * 1024 + 512 * 1024;
    let mut b5 = [0u8; 32];
    let n5 = dump_size_label(mb_half, &mut b5);
    let t5 = core::str::from_utf8(&b5[..n5]).unwrap_or("");
    cs.add(
        "dump_size_rounding",
        t4 == "1.5 KB" && t5 == "1.5 MB",
        "",
    ...
    // 3) 零字节如实 "0 B"。
    let mut b6 = [0u8; 32];
    let n6 = dump_size_label(0, &mut b6);
    let t6 = core::str::from_utf8(&b6[..n6]).unwrap_or("");
