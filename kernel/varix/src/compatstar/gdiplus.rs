...
pub fn run_gdiplus_checks() -> CheckSet {
    CheckSet::merge(run_gdiplus_base_checks(), CheckSet::merge(run_gdiplus_deep_checks(), CheckSet::merge(run_gdiplus_deep2_checks(), CheckSet::merge(run_gdiplus_deep3_checks(), CheckSet::merge(run_gdiplus_deep4_checks(), CheckSet::merge(run_gdiplus_deep5_checks(), CheckSet::merge(run_gdiplus_deep6_checks(), CheckSet::merge(run_gdiplus_deep7_checks(), run_gdiplus_deep8_checks()))))))))
}
...
pub fn rotate90_cw(src: &[u32], w: usize, h: usize, dst: &mut [u32]) -> Option<(usize, usize)> {
    ...
    for sy in 0..h {
        for sx in 0..w {
            ...
            dst[(h - 1 - sy) + sx * h] = src[sy * w + sx]; // dst 新宽 = h
        }
    }
    Some((h, w))
}
