...
// ===========================================================================
// F489 — 全系统闭环自检（CheckSet 汇总，容量 140 覆盖全部域：WP-202 后 74 域，
// WP-205 八域 82 + WP-206 四域 86 + WP-207 三域 89 + WP-209 三域 92 + WP-301
// 四域 96 + WP-302 五域 101 + WP-303 五域 106 + WP-304 三域 109 + WP-305
// 六域 115 + WP-401 六域 121 + WP-402 两域 123 + WP-403 三域 126 + WP-404
// 八域 134（余量 6——按"不够即扩"纪律再扩；KernelCheckup 同理 144 已扩；
// 改域必查第九处口径）
// ===========================================================================

/// 全系统闭环的最大域容量（VARIX 11 域 + TRINITY 20 域 + 判据实装层
/// WP-201/203/208/204/202/205/206/207/209/301/302/303/304/305/401/402/403/404 九十九域 + 余量）。
pub const MAX_LOOP: usize = 140;
...
pub fn run_full_loop() -> FullLoop {
    ...
    // WP-202 · B-901 事件流单向性（三源归一 × 注入接口类型面不存在 C-8 × 过滤器两态不改写）
    lp.register(crate::evflow::run_evflow_checks());
    // WP-202 · B-902 布局表 schema（五字段四层 × 两套内置全表合规 × 切换即时生效）
    lp.register(crate::kblayout::run_kblayout_checks());
    // WP-202 · B-903 IME 三条硬线（最长韵母切分 × 冻结词库档位序 × 100ms/16ms/32MB 预算推导）
    lp.register(crate::imepinyin::run_imepinyin_checks());
    // WP-202 · B-904 组合期分流（仲裁入口直接分流 × 组合期零误触穷举 × 缓冲层编辑）
    lp.register(crate::composesw::run_composesw_checks());
    // WP-202 · B-905 快捷键仲裁（三列词典 × 用户>保留>应用 × 冲突拒绝指名 × 词典唯一）
    lp.register(crate::hkbind::run_hkbind_checks());
    // WP-202 · B-906 候选窗定位（popup_grab 屏幕夹紧 × 下方越界翻上方 × Wine 原生同路径）
    ...
    // WP-206 · B-2001 账本订阅面（六频道×一秒节流×六十点窗口×多消费者对表逐字段一致）
    lp.register(crate::ledgerhub::run_ledgerhub_checks());
    // WP-206 · B-2002 降级原因呈现（四卡与状态机实时一致×受控杀进程×占位诚实×开发者门）
    lp.register(crate::moncards::run_moncards_checks());
    // WP-206 · B-2003 Wine 组透明（组聚合==成员之和×展开独立可见×超限留痕×多组不串账）
    lp.register(crate::winegrp::run_winegrp_checks());
    // WP-206 · B-2103 按钮即评级（兜底级走交接×桥接级标注×推荐位无商业位×安装四段×卸载关联）
    lp.register(crate::starmapui::run_starmapui_checks());
    // WP-207 · B-3901 剪贴板所有权（只发焦点×后台读零成功×读必经合成器×退出即回收×类型上限如实标记）
    // WP-207 · B-3903 大载荷旁路（引用传递微秒级×大图粘贴可取消×文本回显一帧预算）
    lp.register(crate::clipown::run_clipown_checks());
    // WP-207 · B-3902 拖放三取消（载荷定型×光标三态×落点高亮×三取消一清理零泄漏×跨域同协议）
    lp.register(crate::dragdrop::run_dragdrop_checks());
    // WP-207 · B-4001~4005 无障碍五判据（焦点环自动+例外留痕×还焦点协议级×对比度构建期门禁×色弱形状×减弱动效全局）
    lp.register(crate::a11ygate::run_a11y_checks());
    // WP-209 · B-1401~1403 性能基准首轮（27 点位三消费者同环×账本漂移 10‰ 红线×三分位与退化红线）
    lp.register(crate::benchsix::run_benchsix_checks());
    // WP-209 · S209 恢复矩阵组（九类崩溃×七路径×三知情面唯一映射×报告三要素×保护屏五条×重启门）
    lp.register(crate::recovermx::run_recovermx_checks());
    // WP-209 · S210 杀死演练（十姿势零损坏×预算与矩阵同源×门禁捉坏自证）
    lp.register(crate::killdrill::run_killdrill_checks());
    // WP-301 · B-401/404 转译治理边界（四类穷举冻结×未登记默认拒×差异表诚实）
    lp.register(crate::lxgov::run_lxgov_checks());
    // WP-301 · B-403/405 errno 单源与开销（穷举 match 单源×百万次 P95 ≤ 2μs）
    lp.register(crate::lxerrno::run_lxerrno_checks());
    // WP-301 · B-407 伪文件清单（十六文件在册×清单外 ENOENT×内容即时合成）
    lp.register(crate::lxprocfs::run_lxprocfs_checks());
    // WP-301 · B-402/406 vxrun 与载体（四步序×LTP 归因闭环×三类载体×800MB）
    lp.register(crate::lxrun::run_lxrun_checks());
    // WP-302 · B-1001 Wine 会话管家（看门狗失联即重建×百次全恢复×预算同源）
    lp.register(crate::winecare::run_winecare_checks());
    // WP-302 · B-1002 前缀模板（五要素×版本可追溯×重建优于手术×篡改零放行）
    ...
    lp.register(crate::wineattr::run_wineattr_checks());
    // WP-303 · B-1101 SDK 三元组与十二件套矩阵（三元组冻结×双层分工×全构建）
    lp.register(crate::sdktriplet::run_sdktriplet_checks());
    // WP-303 · B-1102 vxapp 清单校验器（对抗样例全拒——违规产物零放行）
    lp.register(crate::sdkmanifest::run_sdkmanifest_checks());
    // WP-303 · B-1103/1104 控件宪章默认与主题 token（全默认×硬编码零例）
    lp.register(crate::sdkwidgets::run_sdkwidgets_checks());
    // WP-303 · B-1105/1106 生命周期托管与调试链路（退出保覆盖 100%×三件齐）
    lp.register(crate::sdkruntime::run_sdkruntime_checks());
    // WP-303 · B-1107 三模板评审门（示例即门面——四要素缺一不发布）
    lp.register(crate::sdktemplate::run_sdktemplate_checks());
    // WP-304 · B-3301 Java 双载体（堆七成设参×futex 首位×SC-J 全绿）
    lp.register(crate::rtjava::run_rtjava_checks());
    // WP-304 · B-3302 Python 双载体（glibc 对齐 Q26×循环检测 Q47×SC-P 全绿）
    ...
    // WP-401 · B-1201/1202 测试金字塔（四层不倒挂闭环×冒烟套判据全对齐）
    lp.register(crate::pyramid::run_pyramid_checks());
    // WP-401 · B-1203/1204 演练量级与实机纪律（十倍量级×白名单双人规则）
    lp.register(crate::drillscale::run_drillscale_checks());
    // WP-401 · B-1205/1206 二十维度证据化（恰好二十条×每日流水红绿通知）
    lp.register(crate::dims20::run_dims20_checks());
    // WP-401 · B-1301/1302 可复现构建（四件套×镜像单源 img/ISO 同源）
    lp.register(crate::reprobuild::run_reprobuild_checks());
    // WP-401 · B-1303/1304 写盘与双槽（四段+令牌×断电五段注入全回退）
    lp.register(crate::imgwrite::run_imgwrite_checks());
    // WP-401 · B-1305/1306 兜底与七步（72h 窗口五分钟回滚×公告模板强制）
    lp.register(crate::rollback72::run_rollback72_checks());
    // WP-402 · B-1404/1405 回归门与基线同步（红即阻断×三处同步+ADR）
    lp.register(crate::reggate::run_reggate_checks());
    // WP-402 · B-2301~2304 体验日志与手册联动（七字段×五信号×红线×手册门）
    lp.register(crate::explog::run_explog_checks());
    // WP-403 · B-1501~1504 安全执法面（能力零越权×W^X×攻击面×四执法点）
    lp.register(crate::secgate::run_secgate_checks());
    // WP-403 · B-1505/1506 签名链与日志红线（三拒×下载段终止×构建期审计）
    lp.register(crate::signchain::run_signchain_checks());
    // WP-403 · B-3101~3103 网络诊断三件套（零干扰抓包×双格式×自动取证）
    lp.register(crate::netdiag::run_netdiag_checks());
    ...
    // WP-404 安装更新与收尾件八域（B-3201~3204 + 收尾组 B-34xx~B-42xx）
    lp.register(crate::instup::run_instup_checks());
    lp.register(crate::fontsub::run_fontsub_checks());
    lp.register(crate::timesrv::run_timesrv_checks());
    lp.register(crate::dispout::run_dispout_checks());
    lp.register(crate::drvframe::run_drvframe_checks());
    lp.register(crate::syslogd::run_syslogd_checks());
    lp.register(crate::qemuenv::run_qemuenv_checks());
    lp.register(crate::idledn::run_idledn_checks());
    lp
}
...
mod tests {
    ...
    fn f489_full_loop_registers_32_domains_all_pass() {
        ...
        // + WP-401 六域 6+5+5+6+5+5 = 32 项 + WP-402 两域 8+14 = 22 项
        // + WP-403 三域 15+7+11 = 33 项
        // （七十五域 CheckSet 条数受各自 all_checks 断言守护）
        assert!(passed >= 39 * 25 + 134 + 68 + 56 + 57 + 57 + 73 + 36 + 28 + 28 + 27 + 33 + 25 + 12 + 27 + 32 + 22 + 33 + 61);
