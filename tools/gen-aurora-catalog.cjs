#!/usr/bin/env node
/**
 * AURORA-10000 领域02 catalog 生成器：
 * 从 docs/AURORA-10000-功能全景图.md 的领域02 段机械生成
 * src/system/windows/aurora/catalog.ts（1250 项 = 50 族 × 25 项）。
 * 用法：node tools/gen-aurora-catalog.cjs
 */
'use strict';
const fs = require('fs');
...
const out = path.join(root, 'src', 'system', 'windows', 'aurora', 'catalog.ts');

const src = fs.readFileSync(doc, 'utf8');
const lines = src.split(/\r?\n/);
const start = lines.findIndex((l) => l.startsWith('## 领域02'));
const end = lines.findIndex((l) => l.startsWith('## 领域03'));
if (start < 0 || end < 0 || end <= start) {
  console.error('未找到领域02/领域03 段边界');
  process.exit(1);
}
...
for (const f of fams) {
  ...
  body += `/** 族${String(n).padStart(4, '0')} ${f.name}（${f.kind} ×25 · 归属：${f.owner}）。 */\n`;
