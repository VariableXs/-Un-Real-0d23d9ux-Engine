...
> **分工包**：AI-J1 = F601-F620（20 项 · 目标 15,660 行上限口径），判据摘自主册《Varix STAR I start.md》第 8 部分 J-A/J-B。生成于本批次收口日；**深化批次二已并入（v2 对账）**；**深化批次三已并入（v3 对账，2026-09-26）**；**深化批次四已并入（v4 对账，2026-09-26 第二批）**；**深化批次五、六已并入（v6 对账，2026-09-27）——九引擎群 + 五处最丑角落真修复**。
> **收口状态**：20/20 项功能落地 + **194 项单测全绿**（j1 63 + j1deep 23 + j1v3 33 + j1v4 27 + j1v5 48）+ **隔离验证门常驻**（j1isolation，随 194 项全量执行）+ 面板/运行时/持久化/遥测/证据/动作路由/对账引擎/**九引擎群**链齐全；行数对账如实呈报（§3 v6）；十二查逐项机器对账（§8，v5 起对账表可机器生成导出）；实机类判据按双轨制登记「随闸门补测」（§5）。
...
## §3 行数对账（v6 · 九引擎群 + 五处 ugly 修复并入后）

- **域功能行现值：7,061 行**（35 个功能文件非空非注释行，同一过滤口径；计数脚本已同步扩充——`_attic/aij1-f601-f620/count-domain-total.sh`），较 v4 收口径（5,638）新增 **1,423 行**。
- **目标上限口径：15,660 行。达成率 45.1%——仍未达「≥90%」铁律，如实呈报不掩盖。**
- 版本对账轨迹：
...
| **v6（本版）** | **7,061** | **194** | **45.1%** | +九引擎群（精确反解/One Euro/Protractor/EDID/拓扑/标定/物理/会话/对账）+五面板+五处 ugly 真修复（同口径净增 1,423） |

*注：v1/v2 行数为当批次报告自报口径；v3 起统一使用归档脚本口径（v2 收口回测 4,169），跨版本可比性以本表为准。

- **v6 剩余缺口定性**：距 90% 口径还差约 7,030 行。真实构成：① 每项判据的实机证据生产（录屏/4K 四档/断电注入——实机日集中产出）；② explorer 标签页历史等跨域消费者的动作接线（需该域协同）；③ 逐项「成熟完整版」第三层 UI 纵深（空态/引导/专家捷径）与 EDID 原始字节通道（系统层设备枚举就绪后消费）。**未完成项已构成下一对话的施工面——批次七按此清单续作，不注水。**
...
```bash
# 单测（194 项：j1.spec 63 + j1deep.spec 23 + j1v3.spec 33 + j1v4.spec 27 + j1v5.spec 48，含 j1isolation 隔离门）
npx vitest run src/features/mouse/__tests__
# 类型检查（本域零错误；仓库既有错误见 §4-16——他人进行中未跟踪文件）
npx tsc --noEmit
# 手工走查入口：设置中心 → 「鼠标」标签页（mouseJ1），十五个纵深子面板折叠展开
...
*AI-J1 · J 鼠标域一分队收口（v6）。九引擎群落地，八窗接线，隔离验证常驻成门，检查项对账可机器生成，数字不注水，缺口与批次七施工面摆在台面上。*
