...
> **分工包**：AI-J1 = F601-F620（20 项 · 目标 15,660 行上限口径），判据摘自主册《Varix STAR I start.md》第 8 部分 J-A/J-B。生成于本批次收口日；**深化批次二已并入（v2 对账）**；**深化批次三已并入（v3 对账，2026-09-26）——跨窗口运行时接线批次**；**深化批次四已并入（v4 对账，2026-09-26 第二批）——大量深化 + 隔离验证自动化 + 十二查机器对账**。
> **收口状态**：20/20 项功能落地 + **146 项单测全绿**（j1 63 + j1deep 23 + j1v3 33 + j1v4 27）+ **隔离验证门 5 项常驻**（j1isolation）+ 面板/运行时/持久化/遥测/证据/动作路由/**对账引擎**七链齐全；八窗口运行时覆盖 + 动作路由中心真实接线；行数对账如实呈报（§3 v4）；十二查逐项机器对账（§8）；实机类判据按双轨制登记「随闸门补测」（§5）。
...
## §3 行数对账（v4 · 大量深化 + 对账层并入后）

- **域功能行现值：5,638 行**（25 个功能文件非空非注释行，同一过滤口径；计数脚本已同步扩充——`_attic/aij1-f601-f620/count-domain-total.sh`），较 v3 收口径（4,844）新增 **794 行**，另有跨域接线 +1（ContextMenu data-wheel 声明）。
- **目标上限口径：15,660 行。达成率 36.0%——仍未达「≥90%」铁律，如实呈报不掩盖。**
- v4 批次的功能构成（全部为判据内真实深化，零注水）：
  - **F617 深化**：手势重绑定（轨迹/动作解耦——「画法是肌肉记忆不该重学，动作才是可自定义面」）+ 墨迹二次贝塞尔平滑（拐角圆顺，视觉品质从工程线到手写笔意）；
  - **F615 深化**：快捷键录制器（按组合键即得映射，修饰键定序归一 + F244 冲突预检——手填示例串退役）；
  - **F607 深化**：屏对护边覆盖全链（SeamGuard pairOverride 执法点 + 屏对面板 + EDID 配对——「按屏对记忆」判据从逻辑层走到 UI 消费）；
  - **F602/F608 体感补全**：慢速微调 HUD（激活即「精修」徽标跟指针——100ms 反馈红线）、磁吸视觉渐近（微滑不瞬移，判定零偏移不变）；
  - **F204/F606 横向惯性通道**：倾斜/Shift 等效横滚共享 F204 余韵（与垂直完全对称的格律哲学）；
  - **章十三 时间轴回放**：分幕回放（间隔切幕）+ 挫败信号归幕——「可回放的操作故事线」从导出 JSON 走到面板步进视图；
  - **检查项对账层**：二十项条目元数据（三件套/落位/路径链/性能线探针/最丑角落一处一事实）+ 十二查机器对账引擎（20×12，探针真执行、实机项如实 gated）+ 十二查对账面板——对账从文档义务变成机器设施（§8 全文）；
  - **F618 真实声明点**：菜单容器 data-wheel="menu"（白名单语义在真组件上生效）；
  - **可发现性**：本页关键词检索（分组级过滤——「忘了在哪时怎么找回」章十一）。
- **v4 质量事件如实记**：F609 性能线探针初版取点踩在阶梯边界（8px=1/3 带宽属慢档）——修正探针取点而非修改实现（实现是判据字面服从，见 §8 查3 与最丑角落自记）。
- **剩余缺口定性**：距 90% 口径还差约 8,400 行。真实构成：① 每项判据的实机证据生产（录屏/4K 四档/断电注入——实机日集中产出）；② explorer 标签页历史等跨域消费者的动作接线（需该域协同）；③ 逐项「成熟完整版」第三层 UI 纵深（空态/引导/专家捷径）。**继续注行数就是注水，我不做。**
...
| v4（本版） | 5,638* | 146 | 36.0% | +重绑定/快捷键录制/屏对面板/横向惯性/回放/十二查对账引擎/隔离验证门（同口径净增 794） |
...
*AI-J1 · J 鼠标域一分队收口（v4）。判据全落地，八窗接线，十二查机器对账，隔离验证常驻成门，数字不注水，缺口摆在台面上。*
