...
## 10. 部署路线拍板落地（2026-09-21 21:46）

- **用户拍板：路线三——干净装 + WE 壁纸库搬家 + 数据走 SHARED**（经两轮问答确认容量账/同步账/内存账后自主选择）。
- 手册同步更新：§0.2 路线定版（含拔盘影响如实声明：壁纸库在 W:，拔盘后内置盘 Steam/WE 显示库不可用）；§0.3 路线三专属前置（内置盘侧 WE 库搬家五步，用户动手）；§3 步骤卡新增 5b（U 盘 Windows 侧 Steam+WE 接线）。
- 数据同源语义确认：SHARED 为同一物理分区非复制同步；WE 壁纸文件搬家即同源；订阅清单在 Steam 账号侧；"哪张壁纸在用"为各机各选（应用配置非文件）；内核世界经 Variable 壁纸中心消费网页壁纸（视频壁纸待第 4 批解码栈）。
- S1.2 执行前待办收束为：用户备料（Hasleo + Win11 ISO，见 §0.1）+ 用户执行 WE 库搬家（§0.3）。备齐后按 §3 步骤卡执行，AI 侧无需再改动流程。

## 11. 发行版定版：消费版 25H2（LTSC 渠道实测结论，2026-09-21 22:11）
