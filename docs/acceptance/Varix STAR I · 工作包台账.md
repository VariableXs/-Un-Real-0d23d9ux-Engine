...
## m2 闸门对账（阶段二出口仪式 · 宿主侧对账 2026-09-24）

**四件套核对**（MD3 行 102/142）：
1. **九包判据全绿**：✅ 判成。全量 ktest **PASS=3490 FAIL=0**（WP-201~209 九包判据 B-501~507 / B-701~707 / B-801~807 / B-601~607 / B-901~907 / B-16xx~19xx / B-2001~2003+2103 / B-3901~4005 / B-1401~1403 首轮+恢复矩阵组，证据三件套各见本台账九份 WP 明细；92 域 CheckSet 全 PASS）。
2. **杀死演练十姿势记录在册**：✅ 判成（宿主语义模型面）。killdrill DrillArchive 十姿势全跑零损坏（Unharmed 4/Recoverable 6/Corrupted 0——零容忍）、rounds 单调+逐姿势记录可重放、预算与恢复矩阵同源（合成器 ≤3s/wineserver ≤5s）、门禁捉坏自证（3500ms 注入必红）；真实 kill -9 注入 ⏳ 实机窗口（S210 = WP-209 + m2 闸门验收，MD3 行 303）。
3. **20 维度验收第一轮全量（逐维证据判定表）**：✅ 判成（宿主证据面）。证据形态按 MD2 篇 12.4——"验收不是打分是交证据"，逐维判定如下：

| 维度 | 判定 | 第一轮证据（宿主侧） |
| --- | --- | --- |
| 1 需求理解 | ✅ | MD1 附录 C.2 对照表随 WP-101/102 明细；25 包判据绑定总表（MD3 4.1）全绑定零悬空 |
| 2 功能完整性 | ✅ | 92 域 CheckSet 全 PASS（记账下限 1512 项之上）+ 判据 1-5 域完成度对账见九份 WP 明细 |
| 3 交互正确性 | ⏳ | WD 走查模板已就位（篇 25 工具链）；WD 走查需实机桌面 |
| 4 细节把控 | ⏳ | 放大三倍截图审——实机窗口 |
| 5 视觉品质 | ◐ | 三主题三缩放矩阵截图 ⏳ 实机；主题 token 对比度构建期门禁宿主 ✅（B-4003） |
| 6 动画品质 | ◐ | 帧序列录制审 ⏳ 实机；减弱动效两档全局零例外宿主 ✅（B-4005） |
| 7 性能 | ◐ | vxbench 骨架首轮 ✅（B-1401~1403：27 点位+三线分布+回归红线，WP-209）；真实计时 ⏳ WP-402 全量 |
| 8 代码质量 | ◐ | 军规风格门禁 0 违规（F479）+ 零第三方依赖（F491）+ ktest 3490 全绿；clippy 全量零告警存档随 WP-401 金字塔 |
| 9 可拓展性 | ◐ | 扩展点清单在册（B-1901 声明式总表新服务零前端/B-804 提交后端 trait/ProbeConsumer 三消费者）；"不改老代码"演练随 WP-401 |
| 10 健壮性 | ✅ | S210 十姿势零损坏 + S209 九类恢复全可达 + 预算常量进代码（WP-209）；真实注入 ⏳ 实机 |
| 11 数据安全 | ✅ | 断电百次 100/100 DRILL-OK（B-703）+ fsync 硬承诺（B-702）+ 半写不入账 + 检查点账本；实机终验随 m4 |
| 12 一致性开放性 | ✅ | VXWM 报文 schema 冻结 + 编号三系互查审计 + 跨域效果位映射公开（B-3902）+ 导出格式验证随 WP-401 |
| 13 文案 | ◐ | 人话三要素文案各域在册（fsview/panicseq/recovermx 范式）；全量导出审随篇 27.1 字符串表脚本化 |
| 14 无障碍 | ◐ | B-4001~4005 五判据实装宿主 ✅（焦点环恒开/还焦点协议级/对比度构建期门禁/形状互异/减弱动效全局）；纯键盘走查+真机对比度实测 ⏳ |
| 15 兼容环境 | ⏳ | 硬件矩阵实测——实机窗口（r3scan 硬件清单固化 + 驱动矩阵第一批在册） |
| 16 安全隐私 | ✅ | 监听默认拒（B-603）+ 剪贴板后台读零成功（B-3901）+ 日志红线类型强制物理化 + VFS 白名单（B-605 咬合）+ 保险箱 AES-GCM |
| 17 错误恢复 | ✅ | 九类崩溃矩阵全可达 + 断电百次 + 防自锁三次（B-706/26.4 承接）+ 失联冻结（判例 17） |
| 18 可观测性 | ◐ | 诊断三件套（B-605）+ 27 点位打点 + 六十点窗口账本（B-2001 同源）；体验日志真实回放演示 ⏳ 实机 |
| 19 文档交付物 | ✅ | MD2 回写段八连（WP-201~209）+ 台账九份明细 + m1/m2 闸门对账 + F487 变更批次记忆；终版 diff 随 WP-405 |
| 20 整体感受 | ⏳ | Variable 本人的真机日用报告——真人项，宿主无法替跑 |

  判定小计：✅ 9 / ◐ 5 / ⏳ 6——宿主可判维度全部判成，实机依赖维度如实标注不提前记绿。
4. **六件套真人走查（MD1 第 21 章七大场景桌面子集）**：⏳ 真人硬项。走查人不是施工者自己（Variable 上手或一位没用过系统的真人）——宿主无法替跑，走查模板已就位待真人记录。
...
## WP-305 收口明细（2026-09-24 · 宿主侧交付 · 判据实装层）

**定性**：星图后端与社区通道（MD3 行 115，B-2101/2102/2104 + B-4301~4303 全六判据）——目录校验链/发布流水线门禁/卸载对账/自测包安全/升级双签/反馈回路落为**判据实装层**，六个新模块（starmapdir/starmapgate/unisweep/selftestpkg/dualsign/feedback），单测前缀 fe14~fe19（避撞验证零占用，selftest 名已被占改 selftestpkg）。两条先行的红线：**星图公信力的技术底座没有"大概是真"的第三种结局**（假目录坏哈希全拒×缺料零上架×缺料拒进人工×越界乱清拒——违规产物零放行的第三次兑现）与**标注==来源、账实相符、标注与签核是恒等与合取不是自觉**（双签查表/改动留理由/关联诚实——防乐观偏差是规则不是裁量）。

**交付面**（六文件新建 + 三文件注册 + 一注释口径 + 一文档回写）：
- `starmapdir.rs`（新建，B-2101 · 5 项）：目录校验链双关（schema 面 id 反域名逐字节——**同源直引 sdkmanifest::valid_id 不重写**+必备字段齐；哈希面 Q55 逐条目逐字节 FNV-1a 指纹重算==声称——**假目录（改一枚字节）与坏哈希全拒**）+ 目录版本一变一号（变更必+1/不变必不动/跳号回退都拒——版本号是时点坐标）+ 星卡引用版本可答（引用版本≤当前——"哪个时点的"永远可答）+ **安装面与评级面分离**（Q83：在册不可装是合法状态——目录是评级事实不是安装白名单）。
- `starmapgate.rs`（新建，B-2102 · 5 项）：流水线三道门依序（**物料三查**：缺截图/复评过期严格不等——当天到期即超期/版本号与实际不符——**任何一项不合拒绝进入人工环节**×**合规三要素**：覆盖度全跑且非空集/指标齐备/偏差解释存在×**人工确认唯一人工环节**）+ 合入版本递增（与 starmapdir::version_step 同源互证，拒绝不占号）+ **缺料星卡零上架**（B-2102 达标线：双失守直落拒绝，人工确认也救不回——门序不可跳）。
- `unisweep.rs`（新建，B-2104 · 4 项）：账本守恒（登记==清理两面相等——漏清计残留）+ **越界乱清拒**（清得比登记多比残留更严重——差额同计入红）+ 关联诚实（MIME 清理数==报告数、自启报了清没报不清——不静默清除）+ **SC-133 全绿**（B-2104 达标线：账实相符+关联诚实双关；Clean/Residual(n) 两态——残留不许含糊）。
- `selftestpkg.rs`（新建，B-4301 · 4 项）：只读形态（can_write 出厂钉死 false——**"不碰系统配置"是结构面不是承诺**；零判例的包也拒）+ 采集范围限定（**B-4301 达标线**：id 相等唯一许可规则——前缀包含也拒，范围是白名单不是氛围）+ 防伪造（环境指纹+时间戳缺一无效——0=没算/没打）+ 复核明示（人工复核+可公开可匿名双明示缺一无效）。
- `dualsign.rs`（新建，B-4302 · 4 项）：降级单签升级双签（**B-4302 达标线**：signoffs_required 查表 2/1/1——调升无第二签不合入，防乐观偏差是查表不是裁量；超额签核只多不少）+ 改动必留理由（override 与 reason 独立合取——理由不是签核的影子，双签也救不回无理由改动）+ 申诉链（附证据+记录结论公开四象限）+ 流程审计（逐行对账一行违规全账红；空账无违规也是合规状态）。
- `feedback.rs`（新建，B-4303 · 5 项）：状态四态穷举（两两互异入判——没有第五种显示）+ 未收录给具体差距（**理由落到数字**：Gap 双计数任一非零即具体，泛泛理由拒——让下一次提交更接近收录，通道才是活的）+ 社区首发标注（**标注==来源恒等**：社区收录必标/非社区多标即红/未收录不许标）+ 全链路可查（**恰好两跳**：已收到→复核中→终态——少一跳没走完多一跳有影子站）+ 季度统计三数（收录率千分比下取整——零提交 None 不编数与 wineattr 同款；收录≤提交自洽）。
- `lib.rs`：六模块注册（rtmpl 后追加，带判据号 doc 注释）；`quality.rs`：六域入 run_full_loop（rtmpl 后）+ 断言链五处同步 109→115（F489 条目+注释/F493 仪表/f489 测试体/F493 测试体/记账下限 +27）+ **MAX_LOOP 110→120 扩容**（109+6=115 破 110——"不够即扩"纪律第四次兑现，余量 5 给阶段四）；`robust.rs`：六域入 checkup；`checks.rs`：容量注释补"WP-305 后 115 域余量 13"口径。
- `docs/Varix STAR I · MD2 技术详案.md`：篇 21 判据表后插入"篇 21 后端与篇 43 通道判据实装回写"段——六判据×模块×CheckSet×实装要点完整表格 + 结构防线两条族 + 勘误连带五条。

**证据三件套**：`cargo kcheck` 绿（既有 warning 与本包无关，新模块零新告警——注意 kcheck 只编 lib 不编 test，测试体编译面由 ktest 覆盖）；全量 `cargo ktest` **PASS=3582 FAIL=0**（较 WP-304 收口 3558 +24 = 六域新单测 24；lib 3582 + fuzz 1+6 + 收尾 0，3558+24 对账吻合）；**CheckSet 27 项**（starmapdir 5/starmapgate 5/unisweep 4/selftestpkg 4/dualsign 4/feedback 5）+ **单测 24 项**（fe14~fe19 各 4）；115 域 CheckSet 全 PASS（F489 `lp.len()==115` + 记账下限 `39*25+134+68+56+57+57+73+36+28+28+27+33+25+12+27`）。复现 = `cd kernel && cargo ktest`；日期 = 2026-09-24。

**红项处置**（ktest 与落刀自查捉住，零带病入库）：
1. **ktest 全量首跑 5 失败单一根因（测试捉住实现 bug——测试网起作用了）**：first_label_ok 首版 `!from_community || labeled` 放行了"非社区收录卡被多标社区首发"——判据语义是标注==来源恒等，改为 `labeled == from_community` 后级联 5 失败（feedback 域→F489→f488/f475/f500）清零，复跑全绿 3582/0。判据语义以 MD2 篇 43.3 为准，实现服从判据。
2. **落刀自查两处死表面（落刀自查即修）**：starmapdir 的 separated() 恒真无人消费（"定义了但没人读"）——删除 impl 改字段面直断；starmapgate 的 GateStage::Received 变体无判据消费——删除（入口状态归 feedback 的 SubmState::Received 管，门禁只裁三站）。
3. **kcheck 编不了 test 的盲区（教训登记）**：separated() 残留调用只被 ktest 的 lib test 编译暴露，kcheck 绿≠测试体编译绿——测试体编译面由 ktest 覆盖，两道检查各有分工不可互替。
4. **后台任务工作目录漂移（施工环境坑三次变体）**：后台跑 cargo ktest 两次 "no such command"——ktest 别名只在 kernel/ 的 .cargo/config 生效，后台命令的 cd 段被剥离，按既有对策 `sh -c 'cd … && cargo ktest'` 包装解决。
5. **fe19 测试体 `let _ =` 假穷举（自查即修）**：标注矩阵首版 8 组合循环体 `let _ =` 全丢弃是假穷举——重写为期望真值表逐格 assert_eq（判据面可见可 grep）。

**环境偏差登记（不阻断，随队跟踪）**：
1. 目录指纹为 FNV-1a u64 宿主模型——真实目录服务哈希链（ksha256 同源）随星图服务接线窗口（B-2101 达标线的宿主形态）。
2. 提交/门禁/复核/反馈全链为账本模型（结构面+状态机）——真实星图仓库侧流水线与更新分发随发布工程窗口（WP-401/405 联动）。
3. 自测包为能力位模型（can_write 钉死+范围白名单）——真实判例引擎轻量分发与采集沙箱随篇 25.1 只读形态窗口。
4. 升级双签为签核计数模型（signoffs_required 查表）——真实双人复核流水线与申诉系统随运营窗口（B-4302 流程审计的实机走查）。

**WP-305 最丑角落（m4 复盘用）**：TwoLedgers 分离为字段面直断（无跨字段约束函数——分离是"不存在推论路径"的负命题，模型面只能穷举组合示证）；物料三查的复评过期以版本日计数（真实日历时间与时区随目录服务接线）；Gap 双计数为差距粒度下限（"缺哪条判例"落到计数，逐条明细的呈现随星图前端详情屏）；signoffs_required 的 Keep=1 为补全穷举的隐含第三态（真实复核规程对维持态是否需签随运营细则定夺）。
- 时序：m2 闸门 ✅ → WP-301 ✅ → WP-302 ✅ → WP-303 ✅ → WP-304 ✅ → WP-305 ✅ → **WP-305 ✅** → 下一站【m3 闸门对账】（五包判据全绿 + LTP 合规子集全过 + Wine 判例自动执行率达标 + 星图收录首批社区提交 + 三条腿交叉走查）。

## m3-compat 闸门对账（2026-09-24 · 宿主侧）

MD3 阶段三出口仪式五门，逐门核账（宿主可验面 vs 实机欠账）：

| 闸门项 | 宿主侧证据 | 实机欠账（随队跟踪） |
| --- | --- | --- |
| 五包判据全绿 | ktest **PASS=3582 FAIL=0**、115 域全 PASS（F489 断言）；WP-301 四域 27 项+WP-302 五域 33 项+WP-303 五域 25 项+WP-304 三域 12 项+WP-305 六域 27 项 = **CheckSet 124 项全过**+单测 96 项（fe01~fe19×4+lx 系）全绿；五包各自 commit（067a675/719948d/b8a8095/44d1c58/bc0c977）全部上云 | 判据的实机达标面（真实判例跑批/LTP 实测）在宿主形态下为登记模型——实机窗口复验 |
| LTP 合规子集全过 | B-402 宿主形态：lxrun 归因闭环（四步序+LTP 归因账本）+封闭依赖清单登记面 | **实机 LTP 跑批**——真机 POSIX 合规子集实测（实机窗口） |
| Wine 判例自动执行率达标 | B-1004/1005 宿主形态：winecase 三档全量流水线模型+promote_to_green 人工门（报告绿+人工确认缺一不可） | **实机判例跑批与自动执行率实测**（实机窗口） |
| 星图收录首批社区提交 | B-4303 宿主形态：feedback 四态+差距面+首发标注+全链路两跳留痕模型已就绪；starmapdir 校验链+starmapgate 门禁可收首批提交 | **首批真实社区提交**（运营窗口——通道是活的的证据） |
| 三条腿交叉走查 | 三腿判据实装层全在：原生（WP-201~209）/兼容（WP-301~305）/Windows 域兜底（WP-206 B-2103 兜底级"切换 Windows 域打开"交接面） | **真人交叉走查**（MD1 第 21 章场景的兼容子集——实机走查窗口） |

**闸门结论**：宿主侧可验面全绿（判据实装层 124 项 CheckSet+96 项单测全过，账目守恒 3558→3582 对账吻合）；LTP 实测/Wine 执行率实测/首批收录/真人走查四项为实机欠账，如实登记不虚判——**m3-compat 宿主侧就绪，实机窗口补四项后判成**。
- 时序：m2 闸门 ✅ → 阶段三 WP-301~305 全 ✅ → **m3 闸门宿主侧对账 ✅（实机四项随队）** → 下一站阶段四 WP-401 测试金字塔与部署发布（B-1201~1206+B-1301~1306，MD3 行 2.4）。

## WP-401 收口明细（2026-09-24 · 宿主侧交付 · 判据实装层）

**定性**：测试金字塔与部署发布（MD3 行 2.4，B-1201~1206 + B-1301~1306 全十二判据）——金字塔四层/冒烟套对齐/演练量级/实机纪律/20 维证据化/每日流水/可复现构建/镜像单源/写盘四段/双槽回滚/72 小时兜底/发布七步落为**判据实装层**，六个新模块（pyramid/drillscale/dims20/reprobuild/imgwrite/rollback72），单测前缀 fe20~fe25（避撞验证零占用）。两条先行的红线：**等式不是下限**（20 维恰好二十条/闭环恰好三步/窗口恰好 72 小时/注入五点全过——达标线是精确对账不是"至少差不多"）与**独立条件不互偿**（写盘确认令牌与段完成/双槽自检与转正——一个条件的绿灯不能替另一个条件作保）。

**交付面**（六文件新建 + 三文件注册 + 一注释口径 + 一文档回写）：
- `pyramid.rs`（新建，B-1201/1202 · 6 项）：四层穷举（层深 0..=3 越下越快越多越早）+ 倒挂泄漏判（depth(发现层)>depth(归属层)即泄漏——方向性谓词，反向与同层不成立）+ 缺口闭环三态（登记→补用例→复验逐级到达，缺级乱序都拒——**B-1201 达标线**）+ 守护编号合法（B-/WD- 起头）+ 冒烟套全对齐（在册+合法，空头与裸奔都拒，空套不算对齐——**B-1202 达标线**）+ 用例缺口可查（判据集合减守护集合=缺口计 1 的实际对账）。
- `drillscale.rs`（新建，B-1203/1204 · 5 项）：十倍量级（实机基数≥1 且 qemu≥real×10——恰好十倍过九倍拒，没跑过实机不谈倍数；**B-1203 达标线**）+ 实机基数门槛（real_runs=0 无论 qemu 多少都拒）+ 脚本六要素合取（遗言/装载/写后必读/幂等+白名单+双人规则——六缺一即红逐项独立验证；**B-1204 达标线**）+ 白名单精确匹配（路径相等唯一许可——父/子/相似名全拒）+ 多场景逐账对账（一场景不足全批红）。
- `dims20.rs`（新建，B-1205/1206 · 5 项）：维度号合法（1..=20）+ 证据化全判（恰好 20 条+无重复+模板实例双全——**B-1205 达标线**，"感觉没问题"不构成证据）+ 数量刚性（19/21 条都拒——等式不是下限）+ 流水四段全自动（构建/测试/报告/通知无人值守）+ 红绿通知（红了必发——只报喜的流水是装饰品；**B-1206 达标线**）。
- `reprobuild.rs`（新建，B-1301/1302 · 6 项）：环境净化四通道（时区/区域/路径/时间戳=提交哈希派生——四条渗入通道全封）+ 可复现四件套合取（工具链锁定+净化+清单哈希+双环境一致——缺一不复现逐件独立红；**B-1301 达标线**）+ 双环境语义（同源不同哈希=名存实亡）+ 布局配置单点（在册+无第二份布局常量）+ 同源衍生（img/ISO 内容零差异——差异只在引导介质形态）+ 单源合取（**B-1302 达标线**）。
- `imgwrite.rs`（新建，B-1303/1304 · 5 项）：写盘四段全过+确认令牌（按序全过+令牌在册——跳段与走过场都拒；**B-1303 达标线**）+ 确认是必要摩擦（令牌与段完成独立——四段全过无令牌仍拒）+ 写备用槽（**写当前槽即红**——附录 I 第 17 条陷阱进类型面）+ 自检转正门（没过自检绝不允许已转正——回滚是自动的不靠用户救砖）+ 断电五段注入全回退（下载/校验/落盘/引导/自检×两种 boot_once 状态全矩阵——**B-1304 达标线**）。
- `rollback72.rs`（新建，B-1305/1306 · 5 项）：回滚窗口语义（转正后恰好 [t, t+72)——整点关闭/旧槽清了即关/转正前倒挂防御）+ 五分钟达标（恰 5 过 6 拒——**B-1305 达标线**）+ 七步穷举（冻结/回归/打包/试运行/发布/公告/复盘步深 0..=6）+ 公告三段式（三段全非空——**空白段不允许发布**；**B-1306 达标线**）+ 已知问题诚实面（"已知问题：无"也是一段——known_issues 位必须显式置真）。
- `lib.rs`：六模块注册（feedback 后追加，带判据号 doc 注释）；`quality.rs`：六域入 run_full_loop（feedback 后）+ 断言链五处同步 115→121（F489 条目+注释/F493 仪表/f489 测试体/F493 测试体/记账下限 +32）+ **MAX_LOOP 120→130 扩容**（115+6=121 破 120——"不够即扩"纪律第五次兑现，余量 9）；`robust.rs`：六域入 checkup（121 域 < MAX_DOMAINS 128 余量 7）；`checks.rs`：容量注释补"WP-401 后 121 域余量 7"口径。
- `docs/Varix STAR I · MD2 技术详案.md`：篇 13 判据表后插入"篇 12 与 13 判据实装回写"段——十二判据×六模块×CheckSet×实装要点完整表格 + 结构防线两条族 + 勘误连带四条。

**证据三件套**：`cargo kcheck` 绿（既有 warning 基线与本包无关，新模块零新告警——grep 六文件名警告数为 0）；全量 `cargo ktest` **PASS=3606 FAIL=0**（较 m3 闸门 3582 +24 = 六域新单测 24；lib 3606 + fuzz 1+6 + 收尾 0，3582+24 对账吻合）；**CheckSet 32 项**（pyramid 6/drillscale 5/dims20 5/reprobuild 6/imgwrite 5/rollback72 5）+ **单测 24 项**（fe20~fe25 各 4）；121 域 CheckSet 全 PASS（F489 `lp.len()==121` + 记账下限 `39*25+134+68+56+57+57+73+36+28+28+27+33+25+12+27+32`）。复现 = `cd kernel && cargo ktest`；日期 = 2026-09-24。

**红项处置**（ktest 与落刀自查捉住，零带病入库）：
1. **`let乱序` 中文标识符语法错（落刀自查即修）**：pyramid.rs 初稿一处 `let乱序 = [...]` 缺空格成非法语法——自查捉住改 `out_of_order`，未上编译器。
2. **imgwrite WriterStage 死表面（落刀自查即修）**：WriterStage 枚举只定义未消费（WriterRun 用下标数组+段名注释承载语义）——删除枚举，段序语义进 doc 注释与判据名。
3. **pyramid 项 6 恒真断言（落刀自查即修）**：`!b701_covered` 以常量集合计算恒真、无对账内容——重写为 want 集合逐编号对账（缺口数==1 是计算结果），断言有实际内容。
4. **dims20 测试数组缺 mut（ktest 编译捉住）**：fe22 中 `no_tpl`/`no_inst` Copy 数组赋值未加 mut——ktest 编译期 E0594 拒绝，加 mut 修复重跑全绿。教训：kcheck（lib）绿不覆盖 test 编译面（m3 前已登记的盲区再次兑现）。

**环境偏差登记（不阻断，随队跟踪）**：
1. 金字塔泄漏判为账本模型（GapState 三态闭环）——真实覆盖缺口统计（行覆盖 60%/翻译函数 80% 实测）随 CI 窗口（B-1201 的实测量）。
2. 演练量级为账面计数（qemu_runs/real_runs 登记）——真实 QEMU 快照断电演练与实机抽测随实机窗口（WP-209 killdrill 联动）。
3. 每日流水为四段自动化登记——真实 CI 流水线与红绿通知随发布工程窗口（B-1206 的实跑）。
4. 双环境比对/断电注入/72h 回滚为模型面（哈希一致布尔/注入点穷举/窗口算术）——真实双环境构建、QEMU 断电百次、回滚演练三记录随 WP-404/405 与实机窗口（m4 出口仪式消费）。

**WP-401 最丑角落（m4 复盘用）**：金字塔层深为静态坐标（真实覆盖率的动态统计随 CI 接线）；十倍量级的 real_runs 为计划抽测量（真实实机抽测基数随实机窗口更新）；写盘四段以布尔数组承载（段间真实 I/O 与进度上报随写盘工具实装——当前是完成账不是执行器）；断电注入五点为枚举穷举（真实 QEMU 快照机制的注入脚本随篇 12.2 冒烟套窗口）。
- 时序：m2 闸门 ✅ → 阶段三全 ✅ → m3 闸门宿主侧对账 ✅ → **WP-401 ✅** → 下一站 WP-402 性能与体验观测体系（B-1401~1405 补全量+B-2301~2304，MD3 行 2.4）。

## WP-402 收口明细（2026-09-24 · 宿主侧交付 · 判据实装层）

**定性**：性能与体验观测体系（MD3 行 2.4，依赖 WP-104 计量钩子/WP-206）——B-1401~1403 已由 WP-209 benchsix 首轮实装（WP-206 契约"一份打点三个消费者"的测量面），本包补全 **B-1404 回归门、B-1405 基线三处同步、B-2301 事件模型、B-2302 挫败信号、B-2303 隐私红线、B-2304 手册联动**六判据，两个新模块（reggate/explog），单测前缀 fe26/fe27（避撞验证零占用）。两条先行的红线：**判定公式单源**（B-1404 直引 benchsix::regress_red——WP-209 的 10% 公式只有一份，编排不重写）与**白名单是类型不是校验**（七字段定长/AggView 无会话槽/Redline 无字节化——三个"不许"全落类型面）。

**交付面**（两文件新建 + 四文件注册/口径 + 两文档回写）：
- `reggate.rs`（新建，B-1404/1405 · 8 项）：改动路径选择表（七规则前缀精确匹配六类全覆盖+未知路径保守全集——宁可多跑不漏跑）+ 红即阻断（合入门子集逐类对基线，性能回归与功能缺陷同罪——**B-1404 达标线**）+ 红项开 issue 开即通知（逐条立票带漂移千分比，红必发）+ 可复现演练（同 seed 重放裁决 bit-for-bit 一致）+ 每日全套对表（B-1206 联动，六类全套同一 regress_red）；三处同步审计（源章节×速查总表×vxbench 基线逐条目全等漂移打回）+ 变更必走 ADR（无备案拒/理由空白拒/old_val 失真拒——**三处同写是同一动作**）+ 组合审计（同步+备案链吻合两关全过——**B-1405 达标线**）。
- `explog.rs`（新建，B-2301~2304 · 14 项）：七字段白名单（8 槽 7 字段定长，扩展字段无处可写）+ 结论事件自带（五档双射穷举，生成那一跳当场裁决无默认——**B-2301 达标线**）+ 扩展走 ADR（唯一入口未备案拒）；五类信号枚举+检测器实判（狂点 2s 四击/死点无反馈/反复开关 3s 六开合/放弃三步后取消终局/错误徘徊窗内原地重复——五类触发+干净序列零误报）+ 升级不复制（原始引用+信号类型）+ 触发率实测标定（对练数据算出 58‰，零扫描 None 不编数——**B-2302 达标线**）；总量轮转（64 槽恒容量写入恒成功）+ 关闭降级明示（Q73 明示是返回值）+ 读取面三层（用户 AggView 类型面无会话槽/开发者按会话回放/审计红线恒零——**B-2303 达标线**）+ 红线类型密封（Redline 零大小无字节化方法，记内容编译不过——B-1506 类型面联动）；手册五段式缺段拒 + 无引用拒收（B/WD/SC/Fix 四类任一，None 与 id=0 都拒——**B-2304 达标线**）+ 联动闭环（B 系引用 id 与判据编号同域互查）。
- `lib.rs`：两模块注册（rollback72 后，带判据号 doc 注释）；`quality.rs`：两域入 run_full_loop + 断言链五处同步 121→123（F489 条目+注释/F493 仪表/f489 测试体/F493 测试体/记账下限 +22）+ MAX_LOOP 130 按兵不动（123 余量 7——**"不够即扩"≠"提前扩"**，按兵不动也是决策）；`robust.rs`：两域入 checkup（123 < MAX_DOMAINS 128 余量 5）；`checks.rs`：容量注释补"WP-402 后 123 域余量 5"。
- `docs/Varix STAR I · MD2 技术详案.md`：篇 14 判据表后与篇 23 判据表后各插入回写段（六判据×两模块×表格+结构防线+勘误连带）。

**证据三件套**：`cargo kcheck` 绿（新模块零新告警基线）；全量 `cargo ktest` **PASS=3618 FAIL=0**（较 WP-401 3606 +12 = fe26×4+fe27×8，对账吻合；fuzz 1+parser 6 亦全绿）；**CheckSet 22 项**（reggate 8/explog 14）+ **单测 12 项**；123 域 CheckSet 全 PASS（F489 `lp.len()==123` + 记账下限 `…+32+22`）。复现 = `cd kernel && cargo ktest`；日期 = 2026-09-24。

**红项处置**（kcheck/ktest 与落刀自查捉住，零带病入库）：
1. **"测试数据要先算一遍被测公式"第八次重演（险情级，落刀自查捉住）**：synth_samples 各类 base 不同（2_000~50_000），回归门测试初稿 baseline 全 10_000 会让多数类天然红；`[11_500;100]` 尾位改 11_600 后 P95 位仍 11_500——全部重算修正（baseline 逐类对齐 base+999 双向对练）。
2. **kcheck 绿≠测试体编译绿第四次兑现**：`*b"A\0\0…"` 11 字节对 [u8;12]（E0308）lib test 编译期暴露——补齐字节串修复。
3. **RedIssue/AdrRecord 缺 Copy derive**：`[None; CAP]` 初始化要求元素 Copy（E0277）——补 derive；同文件 E0507 两处 match 借用随 Copy 一并消解。
4. **检测器对练断言先算窗口**：错误徘徊触发数=失败源数不是重复次数（[1,1,1,1,2]→[1,1,1,1,1]）——落刀自查重算修正。
5. **项 2 baseline 污染项 5 险情（落刀自查捉住）**：CheckSet 内共享可变 baseline 使 SyscallRoundtrip flat 10_000 对 3_000 误红——项 5 改独立基线。

**环境偏差登记（不阻断，随队跟踪）**：
1. 回归判定吃合成样本（宿主模型面）——真实基准计时随实机（WP-209 已登记同族）。
2. 触发率标定为合成事件序列实测——真实用户会话触发率随实机窗口复标（B-2302 实机口径）。
3. 每日对表与合入阻断为编排模型——真实 CI 流水线接线随发布工程窗口（B-1206 联动）。
4. 体验日志为环形模型面——真实落盘与轮转随存储接线；手册毫秒级检索随篇 23.4 实装窗口。
...
- `docs/Varix STAR I · MD2 技术详案.md`：六段回写全插（篇 32/篇 34/篇 35 与 36/篇 37 与 38/篇 41/篇 42——判据×模块×项数×实装要点表格+结构防线+勘误连带+诚实边界；篇 42 注明 B-4301~4303 实装已随 WP-305 收口、终验组织面随 WP-405）。
...
- 时序：m2 闸门 ✅ → 阶段三全 ✅ → m3 闸门宿主侧对账 ✅ → WP-401 ✅ → WP-402 ✅ → WP-403 ✅ → **WP-404 ✅** → 下一站 WP-405 结项复盘与预研立项（20 维度终验+STAR I start 立项+复盘三件套 S405/S406/S407，MD1 第 20/33 章）→ 收官【m4 闸门】。

## WP-405 收口明细（2026-09-24 · 宿主侧交付 · 结项复盘与预研立项）

**定性**：结项复盘与预研立项（S405/S406/S407，MD1 第 20/33 章）——产出是文档与对账不是驱动代码。三件套合订本落盘：`docs/acceptance/Varix STAR I · WP-405 结项复盘与预研立项.md`（S405 二十维度终验终轮+不达标不发布流程声明 / S406 GPU 摸底 R3 立项包三件——硬件清单固化·路径评估·攻坚线排期，只摸不建立场延续 / S407 复盘报告——全偏差谱+教训十族+结构防线资产+三册终版回写对账+实机欠账总清单 / STAR I start 立项草案四主线 / m4-release 闸门宿主侧对账八项）。另：MD2 篇 42 回写段 B-4301~4303 归属表述勘误（实装已随 WP-305 收口，终验组织面随本包——三模块 selftestpkg/dualsign/feedback 与 12 项单测在册核验）。

**证据三件套**：S405 终轮 20 维度逐格判定附证据出处——**✅13/◐3/⏳4**（m2 第一轮 ✅9/◐5/⏳6 → 终轮宿主可判面 +4，性能/代码质量/可拓展性/一致性开放性/文案/可观测性六维以 WP-401~404 新证据判成）；S406 出口形态达成（B-804 三件宿主面 WP-208 已收+本档三件齐）；S407 全偏差谱五账目零悬空（ktest 3256→3662 十九站对账吻合/域 53→134/MAX_DOMAINS 扩容一次/24 包 commit 上云链/CheckSet 记账零赤字）。宿主判据面无新代码——ktest 基线维持 **3662**（fuzz 1+parser 6 全绿）。

**红项处置**：
1. **B-4301~4303 归属臆写（本包自查捉住）**：WP-404 篇 42 回写段误写"随 WP-405 回写"——核验发现三判据已随 WP-305 落刀（selftestpkg/dualsign/feedback 三模块+MD2 篇 43 回写段在册）；MD2 与台账双处勘误，随本包 commit 上云。教训：**回写段的"随 X 包回写"承诺要 grep 核验，不能凭记忆接续**。

**环境偏差登记（不阻断，随队跟踪）**：S405 七格实机/真人维度（3 交互/4 细节/5 视觉半/6 动画半/14 无障碍半/15 兼容/20 整体感受）；m4 两项实机硬项（72h 烤机/回滚演练三次）与断电百次实机终验——全部汇入复盘文档 3.3 实机欠账总清单，随实机窗口逐项补账。

**WP-405 最丑角落（归档用）**：单日全序收口是宿主可判面先走完，不是提前完成——代价是实机欠账集中登记（3.3 清单 16 项），STAR I start 开工前置的实机补账是唯一硬路径；记账下限公式 19 项加数逼近可读性边界（复盘文档已记分段记账议题）；kcheck/ktest 分工盲区五次重演的收敛方案（ktest 定向快跑档）随 m4 复盘议题归档。
- 时序：m1 ✅ → m2 ✅ → m3-compat 宿主侧 ✅（实机四项随队）→ WP-401 ✅ → WP-402 ✅ → WP-403 ✅ → WP-404 ✅ → **WP-405 ✅（S405/S406/S407 三件归档）** → 【m4 闸门】宿主侧对账（见下）。

## m4-release 闸门对账（阶段四出口仪式 · 宿主侧对账 2026-09-24）

**八项逐门核账**（MD3 行 127）：五包判据全绿 ✅（ktest 3662/134 域/四包 commit 0697128·915049b·d545870·f196dc4 上云）；72h 实机烤机 ⏳ 实机；断电百次终验 ◐（宿主 100/100 DRILL-OK m1 收账+PowerDrill 模型，实机终验 ⏳）；回滚演练 ◐（rollback72 三路径模型，实机三次 ⏳）；双签流程审计 ✅（dualsign 逐行对账 B-4302）；20 维度验收终轮 ✅（✅13/◐3/⏳4+不达标不发布声明）；m4-release 两账合一 ✅（任务台账 25 份明细↔判据台账 134 域记账零赤字，性能预算 reggate 常量面在册）；复盘三件套归档 ✅（WP-405 文档随 commit 入 docs/acceptance/）。
...
| 5 | 结项复盘未到时点 | **已完成**：commit e09c7f1（S405 终轮/S406 立项包/S407 复盘+STAR I start 草案/m4 对账八项） |
