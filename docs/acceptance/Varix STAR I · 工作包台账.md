...
**定性**：结项复盘与预研立项（S405/S406/S407，MD1 第 20/33 章）——产出是文档与对账不是驱动代码。三件套合订本落盘：`docs/acceptance/Varix STAR I · WP-405 结项复盘与预研立项.md`（S405 二十维度终验终轮+不达标不发布流程声明 / S406 GPU 摸底 R3 立项包三件——硬件清单固化·路径评估·攻坚线排期，只摸不建立场延续 / S407 复盘报告——全偏差谱+教训十族+结构防线资产+三册终版回写对账+实机欠账总清单 / STAR II 立项草案四主线 / m4-release 闸门宿主侧对账八项）。另：MD2 篇 42 回写段 B-4301~4303 归属表述勘误（实装已随 WP-305 收口，终验组织面随本包——三模块 selftestpkg/dualsign/feedback 与 12 项单测在册核验）。
...
**WP-405 最丑角落（归档用）**：单日全序收口是宿主可判面先走完，不是提前完成——代价是实机欠账集中登记（3.3 清单 16 项），STAR II 开工前置的实机补账是唯一硬路径；记账下限公式 19 项加数逼近可读性边界（复盘文档已记分段记账议题）；kcheck/ktest 分工盲区五次重演的收敛方案（ktest 定向快跑档）随 m4 复盘议题归档。
