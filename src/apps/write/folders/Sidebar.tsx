...
export function Sidebar(): React.ReactElement | null {
  ...
  /** 批次C：跨软件拖拽发生过 → 抑制随后合成的 click 打开文档。 */
  ...
  async function newFolder(parentId: string | null): Promise<void> {
    const v = await askPrompt({
      title: t("newFolder"),
      initial: lang !== "en" ? "新文件夹" : "New folder",
      validate: (v2) => {
        const c = validateItemName(v2);
        return c.ok ? null : c.reason === "empty"
          ? lang !== "en" ? "名称不能为空" : "Name cannot be empty"
          : lang !== "en" ? "名称过长或含非法字符" : "Too long or invalid characters";
      },
    });
    if (!v) return;
    try {
        ...
        onClick: () =>
          void askPrompt({
            ...
            validate: (v) => {
              ...
              return c.ok ? null : lang !== "en" ? "名称无效" : "Invalid name";
            },
          }).then(async (v) => {
            if (!v) return;
            try {
  ...
  function docMenu(d: DocumentMeta, trashed: boolean): MenuItem[] {
    ...
    return [
      { label: lang !== "en" ? "打开" : "Open", icon: <FileText size={14} />, onClick: () => uiStore.setState({ currentDocId: d.id, mode: "write" }) },
      {
      ...
      },
      {
        ...
        onClick: () => window.dispatchEvent(new CustomEvent("variable:create-node-from-doc", { detail: { docId: d.id, title: d.title || (lang !== "en" ? "无标题" : "Untitled") } })),
      },
      {
        label: t("exportDoc"),
        icon: <Download size={14} />,
      ...
      { separator: true },
      {
        ...
        onClick: () =>
          void askConfirm({
            title: t("deleteToTrash"),
            body: `「${d.title || (lang !== "en" ? "无标题" : "Untitled")}」→ ${t("trashView")}？`,
            danger: true,
            okLabel: t("delete"),
          }).then(async (ok) => {
            if (!ok) return;
  ...
  return (
    <aside className="sidebar">
      ...
      {tab !== "workspace" && (
        <div className="doc-list" role="list">
          ...
          <div
            ...
            onMouseDown={(e) => {
              // 批次C（规格 5.7.2）：左键拖动记录 → 跨软件拖拽（拖到 Mind 画布成节点）
              if (tab === "trash" || e.button !== 0) return;
              const sx = e.clientX;
              const sy = e.clientY;
              let started = false;
              const onMove = (ev: MouseEvent): void => {
                ...
                void (async () => {
                  try {
                    const full = await ipc.getDocument(d.id);
                    await beginXDrag("write-record", full.title, {
                      id: full.id, title: full.title, contentHtml: full.contentHtml, contentText: full.contentText,
                    });
                  } catch { /* 文档读取失败 → 放弃本次拖拽 */ }
                })();
              };
          ...
          >
            ...
            <span className="ellipsis doc-title">{d.title || (lang !== "en" ? "无标题" : "Untitled")}</span>
        ...
        {docs.length === 0 && <p className="dim small pad8">{lang !== "en" ? "（空）" : "(empty)"}</p>}
        </div>
      )}
      ...
      {tab !== "trash" && tab !== "workspace" && (
        <>
        <div className="side-section-head">
          <span><MapIcon size={13} /> {lang !== "en" ? "思维导图" : "Mind maps"}</span>
          <span className="flex-1" />
          <button
            type="button"
            className="icon-btn tiny"
            data-tip={lang !== "en" ? "新建导图" : "New map"}
            aria-label={lang !== "en" ? "新建导图" : "New map"}
            onClick={async () => {
              const v = await askPrompt({ title: lang !== "en" ? "新建导图" : "New mind map", initial: t("untitledMap"), validate: (x) => (validateItemName(x).ok ? null : "×") });
              if (!v) return;
