...
export async function ingestProject(
  ...
  onProgress({ phase: "scan", pct: 0.05, detail: lang !== "en" ? "正在扫描项目文件…" : "Scanning project files…" });
  const scan = await ipc.projectScan(root);
  if (scan.entries.length === 0) {
    throw new Error(lang !== "en" ? "这个文件夹里没有可识别的文件" : "No recognizable files in this folder");
  }

  onProgress({ phase: "detect", pct: 0.25, detail: lang !== "en" ? "正在识别项目类型…" : "Detecting project type…" });
  const detect = detectProject(scan);

  onProgress({ phase: "parse", pct: 0.35, detail: lang !== "en" ? "正在解析源代码…" : "Parsing source code…" });
  ...
  for (const src of sources) {
    ...
    if (done % 40 === 0) {
      onProgress({
        ...
        detail: lang !== "en" ? `已解析 ${done}/${sources.length} 个文件` : `Parsed ${done}/${sources.length} files`,
      });
    ...
    try {
      ...
      if (done % 4 === 0) {
        onProgress({
          ...
          detail: lang !== "en" ? `已解析二进制 ${done}` : `Parsed binaries ${done}`,
        });
        await new Promise<void>((r) => window.setTimeout(r, 0));
      }
  ...
  onProgress({ phase: "generate", pct: 0.85, detail: lang !== "en" ? "正在生成思维导图…" : "Generating mind map…" });
  const model: ProjectModel = { scan, detect, analyses, usedBy };
  const graph = buildGraph(model, origin, lang);
  if (graph.nodes.length === 0) {
    throw new Error(lang !== "en" ? "项目太大，导图节点超出上限" : "Project too large: node cap reached");
  }
  onProgress({ phase: "done", pct: 1, detail: lang !== "en" ? `完成：${graph.nodes.length} 个节点` : `Done: ${graph.nodes.length} nodes` });
