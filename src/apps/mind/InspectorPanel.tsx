  ...
  return (
    <div ref={ref} className={`inspector card-pop ${closing ? "closing" : ""}`} onPointerDown={(e) => e.stopPropagation()}>
      <div className="row between">
        <strong className="small">{lang !== "en" ? "属性" : "Properties"}</strong>
        <span>
          <button type="button" className="icon-btn tiny" data-tip={lang !== "en" ? "上移一层" : "Forward"} aria-label="forward" onClick={() => props.onZOrder(1)}><ChevronUp size={12} /></button>
          <button type="button" className="icon-btn tiny" data-tip={lang !== "en" ? "下移一层" : "Backward"} aria-label="backward" onClick={() => props.onZOrder(-1)}><ChevronDown size={12} /></button>
      ...
      <div className="insp-grid">
        ...
        <label>{lang !== "en" ? "宽" : "W"}<input value={w} onChange={(e) => setW(e.target.value)} onBlur={c.w} onKeyDown={(e) => { e.stopPropagation(); if (e.key === "Enter") c.w(); }} /></label>
        <label>{lang !== "en" ? "高" : "H"}<input value={h} onChange={(e) => setH(e.target.value)} onBlur={c.h} onKeyDown={(e) => { e.stopPropagation(); if (e.key === "Enter") c.h(); }} /></label>
      </div>
