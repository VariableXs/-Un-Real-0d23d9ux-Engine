...
import { beginXDrag, useXDropTarget, xfSet } from "../../lib/xflow";
import { copyXrefToClipboard } from "../../lib/xref";
...
// 项目结构可视化引擎（规范一~八章）
import { ingestProject } from "../code/ingest";
import { buildDrillDown, KIND_BORDER, type ProjectModel } from "../code/generate";
import { ProjectImportOverlay, FileInfoCard, type PvImportState } from "../code/ProjectVizPanels";
import type { FileAnalysis, GenGraph } from "../code/types";
...
export function MindmapView(props: { settings: Settings }): React.ReactElement {
  ...
  const flushNodesSave = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      pushToast("error",
        lang !== "en" ? "节点保存失败（内容保留在内存中）" : "Node save failed (kept in memory)",
  ...
  useEffect(() => {
    ...
    async function load(): Promise<void> {
      try {
        ...
        if (!cancelled) pushToast("error", lang !== "en" ? "导图加载失败" : "Failed to load map", errMessage(e).message);
      }
    }
    void load();
    return () => {
  ...
  async function applySnapshot(snap: Snapshot): Promise<void> {
    ...
    try {
      ...
      pushToast("error", lang !== "en" ? "撤销同步失败（内存状态正确）" : "Undo sync failed (in-memory state is correct)", errMessage(err).message);
    }
  }
  ...
  function createNodeAt(wx: number, wy: number, html = ""): MindNode {
    ...
    void ipc.saveNodes([n]).catch((e) =>
      pushToast("error", lang !== "en" ? "创建失败" : "Create failed", errMessage(e).message));
    return n;
  }

  // 批次C（规格 5.7.2）：跨软件拖放 —— Write 记录拖入画布 → 记录标题成为节点。
  const dropHot = useXDropTarget(["write-record"], (clip, cx, cy) => {
    const p = (clip.payload ?? {}) as { title?: string; contentHtml?: string };
    const title = (p.title ?? clip.text).trim().split("\n")[0] ?? "";
    if (!title) return;
    const w = toWorld(cx, cy);
    createNodeAt(w.x - 110, w.y - 30, escapeHtml(title));
    pushToast("success", lang !== "en" ? "已从 Write 导入节点" : "Node imported from Write");
  });
  ...
  async function deleteSelection(): Promise<void> {
    ...
    try {
      ...
      pushToast("error", lang !== "en" ? "删除失败" : "Delete failed", errMessage(e).message);
    }
  }
  ...
  function copySelection(cut = false): void {
    ...
    // 批次C（规格 5.7.1）：富剪贴板 —— Mind 节点粘贴到 Write 变成图片。
    xfSet("mind-node", ns.map((n) => n.textPlain).filter(Boolean).join("\n"), {
  ...
  function pasteClipboard(atWx?: number, atWy?: number): void {
    ...
    void ipc.saveNodes(newNodes).catch((e) => pushToast("error", lang !== "en" ? "粘贴失败" : "Paste failed", errMessage(e).message));
  ...
  async function createEdge(sourceId: string, targetId: string): Promise<void> {
    ...
    if (edges.some((e) => e.sourceNodeId === sourceId && e.targetNodeId === targetId)) {
      pushToast("info", t("connect"), lang !== "en" ? "连接已存在" : "Connection already exists");
      return;
    }
    pushHistory();
    const edge: MindEdge = {
    ...
    try {
      ...
      pushToast("error", lang !== "en" ? "连接保存失败" : "Edge save failed", errMessage(e).message);
    }
  }
  ...
  async function deleteSelectedEdges(): Promise<void> {
    ...
    setSelectedEdges(new Set());
    setEdgePop(null);
    await ipc.deleteEdges(ids).catch((e) => pushToast("error", lang !== "en" ? "删除失败" : "Delete failed", errMessage(e).message));
  }
  ...
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent): void => {
      ...
      if ((e.ctrlKey || e.metaKey) && !e.shiftKey && k === "g" && selection.size > 1) {
        ...
        pushToast("info", lang !== "en" ? `已编组（${selection.size}）` : `Grouped (${selection.size})`);
        return;
      }
      if ((e.ctrlKey || e.metaKey) && e.shiftKey && k === "g" && selection.size > 0) {
        e.preventDefault();
        patchMany(selection, { groupId: null });
        pushToast("info", lang !== "en" ? "已解组" : "Ungrouped");
        return;
      }
  ...
  // 批次C（规格 5.7.3）：引用跳转 —— Write 里点击 Mind 节点引用 → 打开 Mind 并定位节点
  const focusXrefRef = useRef<(id: string) => void>(() => {});
  focusXrefRef.current = (id: string) => {
    if (attemptFocusNode(id, nodesRef.current)) return;
    // 当前导图暂无该节点（可能还在加载）→ 短暂重试
    let tries = 0;
    const iv = window.setInterval(() => {
      if (attemptFocusNode(id, nodesRef.current) || ++tries > 20) window.clearInterval(iv);
    }, 250);
  };
  useEffect(() => {
    let disposed = false;
    ...
    const p = listen<{ kind: string; id: string }>("xref://focus", (ev) => {
      if (ev.payload.kind !== "mind-node") return;
      focusXrefRef.current(ev.payload.id);
    });
    void p.then((u) => {
      if (disposed) u();
      else un = u;
  ...
  function nodeMenuItems(node: MindNode): import("../../components/ContextMenu").MenuItem[] {
    return [
      ...
      ...(node.recordId?.startsWith("pv:")
        ? [{ label: lang !== "en" ? "通俗解读此文件" : "Explain this file", onClick: () => setPvInfoId(node.id) }]
        : []),
      { label: t("copyNode"), onClick: () => { graphClipboard.current = { nodes: [structuredClone(node)], edges: [] }; xfSet("mind-node", node.textPlain, { nodes: [structuredClone(node)], edges: [], mapTitle: map?.name ?? "" }); pushToast("info", t("copied")); } },
      {
        label: lang !== "en" ? "共享到其他软件" : "Share to other app",
        onClick: () => {
          xfSet("mind-node", node.textPlain, { nodes: [structuredClone(node)], edges: [], mapTitle: map?.name ?? "" });
          void beginXDrag("mind-node", node.textPlain, { nodes: [structuredClone(node)], edges: [], mapTitle: map?.name ?? "" });
        },
      },
      {
        // 批次C（规格 5.7.3）：复制为引用 → 在 Write 中粘贴即插入引用锚点
        label: lang !== "en" ? "复制为引用" : "Copy as reference",
        onClick: () => {
          void copyXrefToClipboard("mind-node", node.id, node.updatedAt, node.textPlain).then(() =>
            pushToast("info", lang !== "en" ? "已复制引用，可在 Write 中粘贴" : "Reference copied — paste it in Write"),
          );
        },
      },
      { label: lang !== "en" ? "复制样式" : "Copy style", onClick: () => { styleClipboard.current = { shape: node.shape, borderRadius: node.borderRadius, borderColor: node.borderColor, fillColor: node.fillColor, fontSize: node.fontSize, preset: node.preset, opacity: node.opacity }; pushToast("info", t("styleCopied")); } },
      {
        label: lang !== "en" ? "粘贴样式" : "Paste style",
      ...
      { label: node.hidden ? (lang === "en" ? "Unhide" : "取消隱藏") : (lang === "en" ? "Hide" : "隱藏"), onClick: () => patchNode(node.id, { hidden: !node.hidden }) },
      { label: node.collapsed ? t("expand") : t("collapse"), onClick: () => patchNode(node.id, { collapsed: !node.collapsed }) },
      {
        label: lang !== "en" ? "自由变形" : "Free transform",
      ...
      { separator: true },
      { label: lang !== "en" ? "置顶" : "Bring to front", onClick: () => patchNode(node.id, { zIndex: nodes.reduce((m, n) => Math.max(m, n.zIndex), 0) + 1 }) },
      { label: lang !== "en" ? "置底" : "Send to back", onClick: () => patchNode(node.id, { zIndex: nodes.reduce((m, n) => Math.min(m, n.zIndex), 0) - 1 }) },
      { separator: true },
      { label: t("connect"), onClick: () => { setConnectingFrom(node.id); setSelection(new Set([node.id])); } },
      ...(styleClipboard.current
        ? []
        : []),
      { label: lang !== "en" ? "风格面板…" : "Style panel…", onClick: () => setMenuAnchor({ id: node.id }) },
      { separator: true },
    ...
    const p = getCurrentWebview().onDragDropEvent(async (event) => {
      try {
        ...
        if (projectFiles.length > 0) {
          pushToast("success", lang !== "en" ? "已创建项目档案引用节点" : "Project archive refs created");
        }
        ...
        if (mapFiles.length > 0) {
          pushToast("success", lang !== "en" ? "已创建子导图锚点" : "Sub-map anchor created");
        }
        if (mediaPaths.length > 0) {
          pushToast("success", lang !== "en" ? "已插入媒体" : "Media inserted");
        }
      } catch (e) {
        pushToast("error", lang !== "en" ? "插入媒体失败" : "Insert media failed", errMessage(e).message);
      }
    });
    void p.then((u) => {
      if (disposed) u();
  ...
  function insertTemplate(kind: "mindmap" | "flow" | "swot" | "kanban", at: { x: number; y: number }): void {
    ...
    if (kind === "mindmap") {
      const c = mk(-110, -30, H(lang !== "en" ? "中心主题" : "Central topic"));
      for (let i = 0; i < 4; i++) {
        const a = (i / 4) * Math.PI * 2;
        const child = mk(Math.cos(a) * 320 - 110, Math.sin(a) * 190 - 30, lang !== "en" ? `分支 ${i + 1}` : `Branch ${i + 1}`);
        links.push([c, child]);
      }
    } else if (kind === "flow") {
      ...
      const cols = lang !== "en" ? ["待办", "进行中", "已完成"] : ["To do", "Doing", "Done"];
      cols.forEach((c, i) => {
        mk(i * 300 - 300, -140, H(c), "modern");
        mk(i * 300 - 300, -50, lang !== "en" ? "卡片…" : "Card…", "modern");
      });
    }
    setNodes((prev) => [...prev, ...created]);
    void ipc.saveNodes(created).catch((e) => pushToast("error", lang !== "en" ? "模板插入失败" : "Insert template failed", errMessage(e).message));
  ...
  function gridModeLabel(): string {
    ...
    return m === "dot" ? (lang !== "en" ? "点阵" : "Dots")
      : m === "iso" ? (lang !== "en" ? "等距" : "Isometric")
      : m === "none" ? (lang !== "en" ? "无" : "None")
      : (lang !== "en" ? "方格" : "Grid");
  }
  ...
  async function exportSelected(): Promise<void> {
    ...
    try {
      ...
      pushToast("error", lang !== "en" ? "导出失败" : "Export failed", errMessage(e).message);
    }
  }
  ...
  async function saveToFile(): Promise<void> {
    ...
    try {
      ...
      pushToast("success", lang !== "en" ? "已保存到文件" : "Saved to file", cur);
    } catch (e) {
      pushToast("error", lang !== "en" ? "保存失败" : "Save failed", errMessage(e).message);
    }
  }
  ...
  async function saveMapAs(): Promise<void> {
    ...
    const p = await saveDialog({
      ...
      filters: [
        { name: lang !== "en" ? "思维导图" : "Mindmap", extensions: ["mindmap"] },
    ...
    try {
      ...
      pushToast("success", lang !== "en" ? "已另存为" : "Saved as", p);
    } catch (e) {
      pushToast("error", lang !== "en" ? "另存为失败" : "Save as failed", errMessage(e).message);
    }
  }
  ...
  async function openLocalMindmapFile(path: string): Promise<void> {
    try {
        ...
        if (linked.replace(/\\/g, "/") === norm) {
          try {
            ...
            if (data?.mindmap) {
              ...
              pushToast("info", lang !== "en" ? "已定位到对应导图" : "Located the linked map", norm);
              return;
            }
      ...
      if (parsed.nodes.length === 0) {
        pushToast("info", lang !== "en" ? "该文件是空文档（不含任何节点）" : "This file is an empty document (no nodes)", path);
        return;
      }
      ...
      try {
        ...
        pushToast("success", lang !== "en" ? `已打开「${stem}」` : `Opened "${stem}"`, `${newNodes.length} nodes`);
      } catch (inner) {
  ...
  function importNodesIntoCanvas(parsed: { nodes: unknown[]; edges: unknown[] }): void {
    ...
    void ipc.saveNodes(newNodes).catch((e) => pushToast("error", lang !== "en" ? "导入失败" : "Import failed", errMessage(e).message));
    for (const e of newEdges) void ipc.saveEdge(e).catch(() => {});
  ...
  function createAnchorNode(path: string, clientX?: number, clientY?: number): void {
    ...
    const html = isProject
      ? `<p>📂 <strong>${escapeHtml(stem)}</strong><br><span style="color:#f5c6d8">${lang !== "en" ? "项目档案 · 双击进入分析" : "Project archive · double-click to analyze"}</span></p>`
  ...
  function insertGraph(graph: GenGraph): void {
    ...
    void ipc.saveNodes(created).catch((e) =>
      pushToast("error", lang !== "en" ? "节点保存失败" : "Node save failed", errMessage(e).message));
  ...
  async function importProjectFolder(root: string, at?: { x: number; y: number }): Promise<void> {
    ...
    try {
      ...
      setPvImport(null);
      pushToast("success",
        lang !== "en" ? `项目解读完成（${result.graph.nodes.length} 个节点）` : `Project analyzed (${result.graph.nodes.length} nodes)`,
  ...
  function drillDown(nodeId: string, analysis: FileAnalysis): void {
    ...
    if (fnNodes.length === 0) {
      pushToast("info", lang !== "en" ? "这个文件里没有解析出函数或类" : "No functions/classes parsed in this file");
      return;
    }
    pushHistory();
    ...
    pushToast("success", lang !== "en" ? `已下钻：展开 ${created.length} 个节点` : `Drilled in: ${created.length} nodes`);
  }
    ...
    return (
      <div className="mindmap-view chooser">
        <div className="chooser-card">
          <h3>{t("mapPicker")}</h3>
          {mapsList.length === 0 && <p className="dim">{lang !== "en" ? "还没有思维导图。" : "No mind maps yet."}</p>}
          {mapsList.map((m) => (
  ...
  return (
    <div className="mindmap-view" tabIndex={0} onPointerDown={onCanvasRootPointerDown}>
      <div className="mm-toolbar">
        <div className="seg">
          <button type="button" className={tool === "pan" ? "on" : ""} data-tip={lang !== "en" ? "拖拽平移画布" : "Drag to pan"} aria-label="pan tool" onClick={() => setTool("pan")}><Hand size={14} /></button>
      ...
      <div
        ref={containerRef}
        className={`mm-canvas ${tool}${dropHot ? " xf-hot" : ""}`}
        ...
        onContextMenu={(e) => {
          ...
          openContextMenu(e.clientX, e.clientY, [
            ...
            {
              label: lang !== "en" ? "自动布局" : "Auto layout",
              onClick: () =>
                openContextMenu(e.clientX, e.clientY, [
                  { label: lang !== "en" ? "树状排列" : "Tree", onClick: () => autoLayout("tree") },
                  { label: lang !== "en" ? "网格排列" : "Grid", onClick: () => autoLayout("grid") },
                  { label: lang !== "en" ? "圆形排列" : "Circle", onClick: () => autoLayout("circle") },
                  { label: lang !== "en" ? "力导向排列" : "Force", onClick: () => autoLayout("force") },
                ]),
            },
            {
              label: lang !== "en" ? "整理连接线（正交）" : "Tidy edges (orthogonal)",
              onClick: tidyEdges,
            },
            { separator: true },
            {
              label: lang !== "en" ? "导入项目文件夹…" : "Import project folder…",
              icon: <FolderOpen size={13} />,
              onClick: async () => {
                const sel = await openFileDialog({ directory: true, multiple: false, title: lang !== "en" ? "选择项目文件夹" : "Choose project folder" });
                if (typeof sel === "string" && sel) void importProjectFolder(sel);
              },
            },
            {
              label: lang !== "en" ? "显示全部隐藏框架" : "Show all hidden frames",
            ...
            { label: t("mmSaveAs"), icon: <FileDown size={13} />, onClick: () => void saveMapAs() },
            {
              ...
              onClick: async () => {
                ...
                try {
                  ...
                  pushToast("error", lang !== "en" ? "导出失败" : "Export failed", errMessage(er).message);
                }
              },
            },
          ]);
      ...
      >
        ...
        <div className="mm-status card-pop">
          ...
          <span className="sep">·</span>
          <span>{lang !== "en" ? "选中" : "sel"} {selection.size}</span>
          <span className="sep">·</span>
          <span>{tool === "pan" ? (lang !== "en" ? "抓手" : "Pan") : lang !== "en" ? "框选" : "Select"}</span>
