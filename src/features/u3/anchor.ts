...
import { v4EnginesSelfCheck, v5EnginesSelfCheck } from "./engines";
...
function anchorSelfCheck(): Array<{ name: string; pass: boolean }> {
  ...
  // 十二域注册齐全（九功能域 + anchor 自身 + v4/v5 引擎群——缺域即锚点失义）
  checks.push({ name: "F550 十二域自检注册齐全", pass: U3_ANCHOR_DOMAINS.length === 12 });
  // 功能域检查点总数 ≥ 主册 25 检查点判据（不数 anchor 与 v4/v5 聚合域——防火墙）
  const total = U3_ANCHOR_DOMAINS
    .filter((d) => d.domain !== "anchor" && d.domain !== "v4-engines" && d.domain !== "v5-engines")
...
export const U3_ANCHOR_DOMAINS: Array<{ domain: string; fRange: string; run: () => Array<{ name: string; pass: boolean }> }> = [
  ...
  { domain: "v5-engines", fRange: "F501-F550·v5 装配", run: v5EnginesSelfCheck },
];
