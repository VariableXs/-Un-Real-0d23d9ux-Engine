  ...
  return (
    <div ref={zoneRef} className={`dock-wrap ${shown ? "shown" : ""}`} onPointerDown={(e) => e.stopPropagation()}>
      <div className="dock-bar card-pop">
        {btn(lang === "zh" ? "编辑" : "Edit", <Pencil size={14} />, props.onEdit, !props.hasSel)}
        {btn(lang === "zh" ? `复制 (${props.count})` : `Copy (${props.count})`, <Copy size={14} />, props.onCopy, !props.hasSel)}
        {btn(props.locked ? (lang === "zh" ? "解锁" : "Unlock") : lang === "zh" ? "锁定" : "Lock",
          props.locked ? <LockOpen size={14} /> : <Lock size={14} />, props.onLock, !props.hasSel)}
        {btn(lang === "zh" ? "风格" : "Style", <Palette size={14} />, props.onStyle, props.count !== 1)}
        {btn(lang === "zh" ? "更多" : "More", <MoreHorizontal size={14} />, props.onMore, props.count !== 1)}
        <span className="dock-sep" />
        {btn(lang === "zh" ? "删除" : "Delete", <Trash2 size={14} />, props.onDelete, !props.hasSel)}
      </div>
    </div>
  );
}
