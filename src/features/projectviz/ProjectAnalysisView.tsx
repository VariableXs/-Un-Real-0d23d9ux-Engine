...
export function ProjectAnalysisView(props: { settings: Settings }): React.ReactElement {
  ...
  async function importProjectFolder(root: string): Promise<void> {
    ...
    try {
      ...
      pushToast("success", lang === "zh" ? `项目解读完成（${next.graph.nodes.length} 个节点）` : `Project analyzed (${next.graph.nodes.length} nodes)`, next.detect.label);
    } catch (e) {
      ...
      try {
        ...
        if (!archiveRef.current) {
          const shell: ProjectArchive = {
            ...
            detect: { ...GENERIC_DETECT, label: lang === "zh" ? "一组独立文件" : "a set of loose files" },
        ...
        pushToast("error", lang === "zh" ? `无法读取 ${abs.split(/[\\/]/).pop()}` : `Cannot read file`, errText(e));
      }
    }
    if (added > 0) {
      pushToast("success", lang === "zh" ? `已加入 ${added} 个文件到分析台` : `${added} file(s) added`);
      requestAnimationFrame(() => fitAll());
    }
  }
  ...
  function pinSelected(): void {
    ...
    void (async () => {
      ...
      if (n > 0) pushToast("success", lang === "zh" ? `已固定 ${n} 个文件到画布` : `${n} file(s) pinned`);
      setTreeSel(new Set());
    })();
  }
  ...
  function placeIntentNode(at: { x: number; y: number }): void {
    ...
    const node: GenNode = {
      key,
      html: `<p>💡 <span style="opacity:.65">${lang === "zh" ? "双击输入：我想做一个…" : "Double-click: I want to…"}</span></p>`,
      plain: "",
      kind: "intent",
        ...
        nodes: cur.graph.nodes.map((n) => (n.key === key
          ? {
            ...
            html: trimmed
              ...
              : `<p>💡 <span style="opacity:.65">${lang === "zh" ? "双击输入：我想做一个…" : "Double-click: I want to…"}</span></p>`,
          }
          : n)),
      },
    }));
  ...
  function generateIntent(key: string, text: string): void {
    const a = archiveRef.current;
    const detect = a?.detect ?? { ...GENERIC_DETECT, label: lang === "zh" ? "未导入项目" : "no project" };
  ...
  function drillDown(nodeKey: string, analysis: FileAnalysis): void {
    ...
    if (nodes.length === 0) {
      ...
      pushToast("info", lang === "zh" ? "这个文件里没有解析出函数或类" : "No functions/classes parsed");
      return;
    }
    appendNodes(nodes, edges);
    pushToast("success", lang === "zh" ? `已下钻：展开 ${nodes.length} 个节点` : `Drilled in: ${nodes.length} nodes`);
  }
  ...
  async function refToMindmap(relPath: string, name: string, label: string): Promise<void> {
    ...
    if (!a) return;
    if (!mapId) {
      pushToast("error", lang === "zh" ? "请先打开一个思维导图" : "Open a mind map first");
      return;
    }
    if (a.refs.some((r) => r.relPath === relPath && r.mapId === mapId)) {
      pushToast("info", lang === "zh" ? "该文件已引用到当前导图" : "Already referenced");
      return;
    }
    const node: MindNode = {
      id: uid(),
    ...
    try {
      ...
      pushToast("error", lang === "zh" ? "引用节点创建失败" : "Ref node failed", errText(e));
      return;
    }
    patchArchive((cur) => ({
      ...cur,
    ...
    pushToast("success", lang === "zh" ? "已引用到当前思维导图（双击该节点可跳回解读）" : "Referenced into the mind map");
  ...
  async function loadArchiveFile(path: string): Promise<void> {
    try {
      ...
      if (!parsed || parsed.formatVersion !== 1 || !parsed.graph) {
        throw new Error(lang === "zh" ? "不是有效的 .project 档案" : "Not a valid .project archive");
      }
      ...
      setFilePath(path);
      requestAnimationFrame(() => fitAll());
      pushToast("success", lang === "zh" ? "项目档案已打开" : "Project archive opened", parsed.root);
    } catch (e) {
      pushToast("error", lang === "zh" ? "档案打开失败" : "Open archive failed", errText(e));
    }
  }
  ...
  async function exportGraphMindmap(): Promise<void> {
    ...
    try {
      ...
      pushToast("success", lang === "zh" ? "分析结果已导出为思维导图" : "Analysis exported as mindmap", p);
    } catch (e) {
      pushToast("error", lang === "zh" ? "导出失败" : "Export failed", errText(e));
    }
  }
  ...
  async function writeArchive(path: string): Promise<void> {
    ...
    try {
      ...
      pushToast("success", lang === "zh" ? "档案已保存" : "Archive saved", path);
    } catch (e) {
      pushToast("error", lang === "zh" ? "保存失败" : "Save failed", errText(e));
    }
  }
  ...
  async function openFocus(rel: string, root: string, level: 2 | 3 | 4 = 2, symbol?: string): Promise<FocusState | null> {
    ...
    if (content === null) {
      try {
        ...
        pushToast("error", lang === "zh" ? "无法读取源码" : "Cannot read source", errText(e));
        return null;
      }
    }
  ...
  function runInlineIntent(): void {
    ...
    if (bar.mode === "note") {
      ...
      pushToast("success", lang === "zh" ? "注释已保存到项目档案" : "Note saved to archive");
      setInlineBar(null);
      return;
    }
    const r: IntentResult = applyIntent(text, { content: f.content, lang: f.lang, line: bar.line, relPath: f.rel });
    if (!r.matched) {
      pushToast("error", r.note || (lang === "zh" ? "没有匹配到可执行的改动" : "No applicable change"), r.clarify ?? undefined);
      return;
    }
    ...
    pushToast("success", lang === "zh" ? "已应用到会话——点右上角「写入源文件」落盘" : "Applied in session — use 'Write to source' to persist", r.note);
    setInlineBar(null);
  }
  ...
  async function writePendingToFile(): Promise<void> {
    ...
    try {
      ...
      pushToast("error", lang === "zh" ? "写入失败（文件未改动）" : "Write failed (file untouched)", errText(e));
      return;
    }
    ...
    flash(`${p.rel}:${p.line}`);
    pushToast("success", lang === "zh" ? "已写入源文件并自动备份（Ctrl+Z 可回滚）" : "Written & backed up (Ctrl+Z to undo)", p.note);
  }
  ...
  function undoLastEdit(): void {
    ...
    if (p && focusRef.current?.rel === p.rel) {
      ...
      setPendingWrite(null);
      pushToast("success", lang === "zh" ? "已撤销会话内改动（尚未写盘）" : "In-session change reverted");
      return;
    }
    const rec = editHistoryRef.current.pop();
    if (!rec) {
      pushToast("info", lang === "zh" ? "没有可回滚的编辑" : "Nothing to undo");
      return;
    }
    void (async () => {
      try {
        ...
        pushToast("success", lang === "zh" ? "已回滚上一次编辑" : "Last edit reverted", rec.utterance);
      } catch (e) {
        pushToast("error", lang === "zh" ? "回滚失败" : "Undo failed", errText(e));
      }
    })();
  }
  ...
  function openReasoning(f: FocusState, symbol: string): void {
    ...
    if (!chains) {
      pushToast("info", lang === "zh" ? "没有解析出这个函数" : "Symbol not parsed");
      return;
    }
  ...
  function symbolMenu(e: React.MouseEvent, name: string): void {
    ...
    openContextMenu(e.clientX, e.clientY, [
      { label: lang === "zh" ? "查看推理链（三条）" : "Reasoning chains", onClick: () => openReasoning(f, name) },
      { label: lang === "zh" ? "进入段落级（L3）" : "Paragraphs (L3)", onClick: () => setFocus({ ...f, level: 3, symbol: name }) },
    ]);
  }
  ...
  function lineMenu(e: React.MouseEvent, card: LineCard): void {
    ...
    if (!f) return;
    openContextMenu(e.clientX, e.clientY, [
      { label: lang === "zh" ? "在此处续写…" : "Continue here…", onClick: () => setInlineBar({ mode: "continue", line: card.line, text: "" }) },
      { label: lang === "zh" ? "用大白话改这一行…" : "Change with plain words…", disabled: !card.editable, onClick: () => setInlineBar({ mode: "edit", line: card.line, text: "" }) },
      { label: lang === "zh" ? (archiveRef.current?.notes[`${f.rel}:${card.line}`] ? "编辑个人注释…" : "添加个人注释…") : (archiveRef.current?.notes[`${f.rel}:${card.line}`] ? "Edit note…" : "Add note…"), onClick: () => setInlineBar({ mode: "note", line: card.line, text: archiveRef.current?.notes[`${f.rel}:${card.line}`] ?? "" }) },
      { label: lang === "zh" ? "复制该行" : "Copy line", onClick: () => void navigator.clipboard.writeText(card.code).then(() => pushToast("success", lang === "zh" ? "已复制" : "Copied")).catch(() => {}) },
    ]);
  }
  ...
  function canvasMenu(x: number, y: number): void {
    openContextMenu(x, y, [
      { label: lang === "zh" ? "导入项目文件夹…" : "Import project folder…", icon: <FolderOpen size={13} />, onClick: () => void pickFolder() },
      { label: lang === "zh" ? "加入文件到分析台…" : "Add files to session…", onClick: () => void pickFiles() },
      { separator: true },
      { label: lang === "zh" ? "用大白话加功能（单击画布放置意图）" : "Plain-language intent (click canvas)", checked: toolMode === "intent", onClick: () => setToolMode(toolMode === "intent" ? "select" : "intent") },
      { separator: true },
      { label: lang === "zh" ? "适应全部" : "Fit all", onClick: fitAll },
      { label: lang === "zh" ? "保存档案" : "Save archive", disabled: !archive, onClick: () => void saveArchive() },
      { label: lang === "zh" ? "档案另存为…" : "Save archive as…", disabled: !archive, onClick: () => void saveArchiveAs() },
    ]);
  }
  ...
  function nodeMenu(e: React.MouseEvent, key: string): void {
    ...
    if (n.recordId?.startsWith("pv:")) {
      items.push({ label: lang === "zh" ? "通俗说明" : "Explain", onClick: () => setPod({ kind: "file", recordId: n.recordId! }) });
      items.push({
        label: lang === "zh" ? "逐行解剖（L2 结构级）" : "Line anatomy (L2)",
        onClick: () => {
          const body = n.recordId!.slice(3);
          const at2 = body.lastIndexOf("@");
      ...
      items.push({ label: lang === "zh" ? "引用到思维导图" : "Ref to mindmap", onClick: () => void refToMindmap(rel, rel.split("/").pop() ?? rel, "") });
      items.push({ label: lang === "zh" ? "从分析中移除" : "Remove from session", danger: true, onClick: () => removeNode(key) });
    } else if (n.kind === "intent") {
      items.push({ label: lang === "zh" ? "编辑大白话" : "Edit intent", onClick: () => setEditingKey(key) });
      if (n.plain) items.push({ label: lang === "zh" ? "重新生成" : "Regenerate", onClick: () => generateIntent(key, n.plain) });
      items.push({ label: lang === "zh" ? "删除意图分支" : "Delete intent", danger: true, onClick: () => removeNode(key) });
    } else {
      items.push({ label: lang === "zh" ? "从画布移除" : "Remove from canvas", danger: true, onClick: () => removeNode(key) });
    }
  ...
  async function pickFolder(): Promise<void> {
    try {
      const sel = await fileDialog({ directory: true, multiple: false, title: lang === "zh" ? "选择项目文件夹" : "Choose project folder" });
      if (typeof sel === "string" && sel) await importProjectFolder(sel);
    } catch (e) {
      pushToast("error", lang === "zh" ? "打开文件夹选择器失败" : "Folder dialog failed", errText(e));
    }
  }

  async function pickFiles(): Promise<void> {
    try {
      const sel = await fileDialog({ multiple: true, title: lang === "zh" ? "加入文件到分析台" : "Add files" });
      const paths = Array.isArray(sel) ? sel : typeof sel === "string" ? [sel] : [];
      if (paths.length > 0) await importLooseFiles(paths);
    } catch (e) {
      pushToast("error", lang === "zh" ? "打开文件选择器失败" : "File dialog failed", errText(e));
    }
  }
    ...
    return (
      <div className="pv-anatomy" key={`${f.rel}-${f.level}-${f.symbol ?? ""}`}>
        <div className="pv-crumb-bar">
          <button type="button" className="btn tiny ghost" onClick={popFocusLevel}><ArrowLeft size={11} /> {lang === "zh" ? "返回上一层" : "Back"}</button>
        ...
        <div className="pv-anatomy-col">
          {f.level === 2 && (
            <>
              <p className="pv-anatomy-hint">{lang === "zh" ? "L2 结构级：双击进入下级；右键函数看推理链" : "L2: double-click to drill; right-click functions for chains"}</p>
              {symbols.length === 0 && blocks.length === 0 && (
                <p className="dim pad8">{lang === "zh" ? "文件为空。" : "Empty file."}</p>
              )}
              {symbols.map((s) => (
                <div
                  key={s.name}
                ...
                >
                  ...
                  <div className="pv-line-explain">{s.params.length > 0 ? `原材料：${s.params.join("、")}` : (lang === "zh" ? "不需要原材料" : "no input")}</div>
                </div>
              ))}
              {blocks.map((b) => (
                <div
                ...
                >
                  <div className="pv-line-code">
                    <span className="pv-badge">{lang === "zh" ? "块" : "blk"}</span>
          ...
          {f.level === 3 && sym && (
            <>
              <p className="pv-anatomy-hint">{lang === "zh" ? `L3 段落级：${sym.name}() 的逻辑分块——双击进入逐行（L4）` : "L3 paragraphs — double-click for line view"}</p>
              {chunkBody(bodyLines).map((ch) => (
                ...
                >
                  <div className="pv-line-code"><span className="pv-badge">{lang === "zh" ? "段落" : "chunk"}</span><strong>{ch.title}</strong> <span className="dim small">L{bodyStart + ch.start}</span></div>
          ...
          {(f.level === 4 || (f.level === 3 && !sym)) && (
            <>
              <p className="pv-anatomy-hint">
                {lang === "zh" ? "L4 逐行级：左代码右大白话；双击大白话反写，点击函数行折叠 · " : "L4 dual-track · "}
                <button type="button" className="btn tiny ghost" onClick={() => setCollapsedSyms(new Set(symbols.map((s) => s.name)))}>{lang === "zh" ? "折叠所有函数" : "Fold all"}</button>
                <button type="button" className="btn tiny ghost" onClick={() => setCollapsedSyms(new Set())}>{lang === "zh" ? "展开所有" : "Expand all"}</button>
              </p>
              {renderL4Virtual(f)}
            </>
          )}
  ...
  function buildL4Rows(f: FocusState): VRow[] {
    ...
    if (lines.length > cap) {
      rows.push({ kind: "more", text: lang === "zh" ? `仅展开前 ${cap} 行（其余 ${lines.length - cap} 行）` : `First ${cap} lines shown` });
    }
    return rows;
  }
            ...
            if (row.kind === "sym" || row.kind === "block") {
              ...
              return (
                ...
                >
                  ...
                  <div className="pv-line-explain" title={lang === "zh" ? "点击折叠/展开" : "click to fold"}>
                    <p>{isSym ? row.goal : (lang === "zh" ? `看情况办事/重复劳动的一段（至 L${row.endLine}），折叠可只留总结` : `conditional/loop block to L${row.endLine}`)}{folded ? (lang === "zh" ? "（已折叠）" : " (folded)") : ""}</p>
                  </div>
                </div>
              );
            }
            ...
            return (
              ...
              >
                ...
                <div
                  ...
                  title={lang === "zh" ? "双击大白话 → 用自然语言反向修改这一行" : "double-click to edit this line with plain words"}
                >
                  <p>{card.plain || " "}</p>
                  ...
                  {card.impact.length > 0 && <p className="impact">{lang === "zh" ? "影响：" : "Impact: "}{card.impact.join("；")}</p>}
                  {!card.editable && <span className="pv-badge warn">{lang === "zh" ? "慎重" : "caution"}</span>}
                </div>
              </div>
            );
          })}
  ...
  return (
    <div className="project-view">
      ...
      <div className="pv-toolbar">
        <button type="button" className="icon-btn tiny" data-tip={lang === "zh" ? "返回写作" : "Back to writing"} onClick={() => uiStore.setState({ mode: "write" })}>
          <PenLine size={14} />
        </button>
        <span className="pv-title"><FolderSearch size={14} /> {lang === "zh" ? "项目分析" : "Project"}</span>
        <span className="pv-crumb" title={archive?.root ?? ""}>
          {archive ? `${rootName} ▸ ${lang === "zh" ? "项目根" : "root"}` : (lang === "zh" ? "未导入项目" : "No project")}
        </span>
        {archive && <span className="pv-crumb dim">{lang === "zh" ? `${pinnedCount} 个文件已纳入` : `${pinnedCount} file(s) in session`}</span>}
        <span className="flex-1" />
        <button type="button" className="icon-btn tiny" data-tip={lang === "zh" ? "文件树" : "File tree"} onClick={() => setTreeOpen(!treeOpen)}>
          {treeOpen ? <PanelLeftClose size={14} /> : <PanelLeftOpen size={14} />}
        </button>
        <button type="button" className={`icon-btn tiny ${toolMode === "intent" ? "active" : ""}`} data-tip={lang === "zh" ? "用大白话加功能（单击画布放置意图节点）" : "Plain-language intent"}
          onClick={() => setToolMode(toolMode === "intent" ? "select" : "intent")}>
          <Lightbulb size={14} />
        </button>
        <button type="button" className="icon-btn tiny" data-tip={lang === "zh" ? "导入/重新扫描项目文件夹" : "Import / rescan folder"} onClick={() => void pickFolder()}>
          <FolderOpen size={14} />
        </button>
        <button type="button" className="icon-btn tiny" data-tip={lang === "zh" ? "加入文件到分析台" : "Add files"} onClick={() => void pickFiles()}>
          <Pin size={14} />
        </button>
        <button type="button" className="icon-btn tiny" data-tip={lang === "zh" ? "导出为思维导图 (.mindmap)" : "Export as mindmap"} disabled={!archive} onClick={() => void exportGraphMindmap()}>
          <GitBranch size={14} />
        </button>
        <button type="button" className="icon-btn tiny" data-tip={lang === "zh" ? "保存档案 (.project)" : "Save archive (.project)"} disabled={!archive} onClick={() => void saveArchive()}>
          <Save size={14} />
        </button>
        <button type="button" className="icon-btn tiny" data-tip={lang === "zh" ? "档案另存为" : "Save archive as"} disabled={!archive} onClick={() => void saveArchiveAs()}>
          <FileDown size={14} />
        </button>
        <button type="button" className="icon-btn tiny" data-tip={lang === "zh" ? "重新扫描" : "Rescan"} disabled={!archive} onClick={() => void importProjectFolder(archive!.root)}>
          <RefreshCw size={14} />
        </button>
        <button type="button" className="icon-btn tiny" data-tip={lang === "zh" ? "适应全部" : "Fit all"} disabled={!archive} onClick={fitAll}>
          <Maximize2 size={14} />
        </button>
        <button type="button" className="icon-btn tiny" data-tip={lang === "zh" ? "撤销上一次代码写入（Ctrl+Z）" : "Undo last write (Ctrl+Z)"} onClick={undoLastEdit}>
          <Undo2 size={14} />
        </button>
        <button type="button" className="icon-btn tiny" data-tip={lang === "zh" ? "字典管理（L1-L5）" : "Dictionary manager"} onClick={() => setPod({ kind: "dict" })}>
          <BookOpen size={14} />
        </button>
        {pendingWrite && (
          <button type="button" className="btn tiny primary pv-save-btn" onClick={() => void writePendingToFile()}>
            <Save size={12} /> {lang === "zh" ? "写入源文件" : "Write to source"}
          </button>
        )}
        <button type="button" className={`icon-btn tiny ${refsOpen ? "active" : ""}`} data-tip={lang === "zh" ? "双向引用" : "Cross references"} disabled={!archive} onClick={() => setRefsOpen(!refsOpen)}>
          <Link2 size={14} />
        </button>
        ...
        <select
          ...
          value={narrateStyle}
          aria-label={lang === "zh" ? "通俗风格" : "Narration style"}
          onChange={(e) => changeStyle(e.target.value as NarrateStyle)}
        >
          <option value="metaphor">{lang === "zh" ? "生活比喻" : "Metaphor"}</option>
          <option value="story">{lang === "zh" ? "故事叙事" : "Story"}</option>
          <option value="engineering">{lang === "zh" ? "工程说明" : "Engineering"}</option>
        </select>
      </div>
      ...
      <div className="pv-body">
          ...
          <div className="pv-tree">
            <div className="pv-tree-head">
              <input
                ...
                value={treeFilter}
                placeholder={lang === "zh" ? "筛选文件…" : "Filter files…"}
            ...
            <div className="pv-tree-list">
              {treeRows.length === 0 && (
                <p className="dim small pad8">{archive ? (lang === "zh" ? "（无匹配文件）" : "(no match)") : (lang === "zh" ? "导入项目后显示文件树" : "Import a project first")}</p>
              )}
              {treeRows.map((row) => (
                <div
                  key={row.rel}
                ...
                >
                  ...
                  {row.isExternal && <span className="pv-tag">{lang === "zh" ? "外" : "ext"}</span>}
                </div>
              ))}
            </div>
            <div className="pv-tree-foot">
              <button type="button" className="btn tiny ghost" disabled={treeSel.size === 0} onClick={pinSelected}>
                <Pin size={11} /> {lang === "zh" ? "固定到画布" : "Pin"}
              </button>
              <span className="dim small">{treeSel.size} {lang === "zh" ? "选中" : "sel"}</span>
            </div>
          </div>
        )}
        ...
        >
          {!archive && !focus && (
            <div className={`pv-empty ${dropHover ? "hot" : ""}`}>
              <p>{lang === "zh" ? "拖入项目文件夹或源文件开始分析" : "Drop a project folder or source files to analyze"}</p>
              <button type="button" data-interactive className="btn ghost" disabled={!!pvImport} onClick={() => void pickFolder()}>
                <FolderOpen size={14} /> {lang === "zh" ? "导入项目或文件" : "Import project or files"}
              </button>
            </div>
          )}
            ...
            <div className="pv-world" style={{ transform: `translate(${vp.x}px, ${vp.y}px) scale(${vp.z})`, transformOrigin: "0 0" }}>
              ...
              {(() => {
                ...
                >
                  ...
                  {editingKey === n.key && (
                    <div className="pv-intent-hint">{lang === "zh" ? "Enter 生成 · Esc 取消" : "Enter to generate · Esc to cancel"}</div>
                  )}
                </div>
                  ));
              })()}
          ...
          {archive && refsOpen && (
            <div className="pv-refs card-pop">
              <div className="pv-info-head">
                <Link2 size={13} />
                <strong>{lang === "zh" ? "双向引用管道" : "Cross references"}</strong>
          ...
          <div className="pv-status card-pop">
            <span>{focus ? `L${focus.level}` : (archive ? archive.graph.nodes.length : 0)}{focus ? "" : ` ${lang === "zh" ? "节点" : "nodes"}`}</span>
            <span className="sep">·</span>
            <span style={{ color: CROSS_REF_COLOR }}>{archive?.refs.length ?? 0} {lang === "zh" ? "引用" : "refs"}</span>
            <span className="sep">·</span>
            <span>{toolMode === "intent" ? (lang === "zh" ? "意图模式：单击画布放置" : "Intent mode: click canvas") : `${Math.round(vp.z * 100)}%`}</span>
            <span className="sep">·</span>
          ...
          {inlineBar && (
            <div className="pv-inline-bar card-pop">
              <span className="pv-badge">{inlineBar.mode === "continue" ? (lang === "zh" ? `在第 ${inlineBar.line} 行后续写` : `after L${inlineBar.line}`) : inlineBar.mode === "note" ? (lang === "zh" ? `给第 ${inlineBar.line} 行加个人注释` : `note L${inlineBar.line}`) : (lang === "zh" ? `改第 ${inlineBar.line} 行` : `edit L${inlineBar.line}`)}</span>
              <input
                autoFocus
                value={inlineBar.text}
                placeholder={inlineBar.mode === "note" ? (lang === "zh" ? "写下你对这一行的理解…" : "your note for this line…") : (lang === "zh" ? "大白话，如：加个日志写上“用户尝试登录” / 改成蓝色 / 把 userName 改名叫 userId" : "e.g. add a log / change to blue / rename x to y")}
                ...
                if (preview) {
                  return (
                    <span className="pv-inline-preview" title={preview.after}>
                      {lang === "zh" ? "预览：" : "Preview: "}{preview.before ? `第 ${preview.startLine} 行` : `第 ${preview.startLine} 行后`} · {preview.note}
                    </span>
                  );
                }
        ...
        {pod?.kind === "reasoning" && (
          <div className="pv-pod pv-intent-pod">
            <div className="pv-info-head">
              <Lightbulb size={13} />
              <strong>{pod.symbol}() · {lang === "zh" ? "三条推理链" : "reasoning chains"}</strong>
            ...
            <div className="pv-info-body">
              <section>
                <h4>{lang === "zh" ? "纵向推理链（从目标到实现）" : "Vertical"}</h4>
                <p>{lang === "zh" ? "目标：" : "Goal: "}{pod.chains.vertical.goal}</p>
              ...
              <section>
                <h4>{lang === "zh" ? "横向依赖链（与其他函数的关系）" : "Horizontal"}</h4>
                <ul className="pv-list">
                  ...
                  {pod.chains.horizontal.peers.length > 0 && (
                    <li key="p">{lang === "zh" ? "平级协作：" : "Peers: "}{pod.chains.horizontal.peers.join("、")}</li>
                  )}
                </ul>
              </section>
              <section>
                <h4>{lang === "zh" ? "时间因果链（先什么再什么最后什么）" : "Temporal"}</h4>
                <p>{lang === "zh" ? "入口：" : "Entry: "}{pod.chains.temporal.entry}</p>
                <ol className="pv-list pv-steps">
...
function IntentPod(props: {
  ...
  async function copyCode(): Promise<void> {
    ...
    try {
      ...
      pushToast("success", lang === "zh" ? "代码已复制" : "Code copied");
    } catch {
      pushToast("error", lang === "zh" ? "复制失败" : "Copy failed");
    }
  }
  async function writeToFile(): Promise<void> {
    ...
    try {
      ...
      pushToast("success", lang === "zh" ? "已创建新文件（不覆盖已有文件）" : "New file created (never overwrites)", path);
    } catch (e) {
      pushToast("error", lang === "zh" ? "写入失败" : "Write failed", errText(e));
    }
  }
  return (
    <div className="pv-pod pv-intent-pod">
      <div className="pv-info-head">
        <Lightbulb size={13} />
        <strong>{p.title}{p.matched ? "" : lang === "zh" ? "（未命中模板）" : " (no template)"}</strong>
      ...
      <div className="pv-info-body">
        {props.noProjectHint && (
          <p className="pv-warn-text">
            {lang === "zh" ? "先导入项目，我才能知道该写进哪个文件。" : "Import a project first so I know where to write."}
            <button type="button" className="btn tiny ghost" onClick={props.onImportFirst}>
              <FolderOpen size={11} /> {lang === "zh" ? "导入后再生成" : "Import first"}
            </button>
          </p>
        )}
        <ol className="pv-list pv-steps">
        ...
        {p.unknownTerms.length > 0 && (
          <p className="pv-warn-text">{lang === "zh" ? `这些词我还不太懂：${p.unknownTerms.join("、")}` : `Unclear: ${p.unknownTerms.join(", ")}`}</p>
        )}
        {p.code && (
          <>
            <pre className="pv-code"><code>{p.code}</code></pre>
            <p className="pv-detail">{lang === "zh" ? "建议粘贴位置：" : "Paste location: "}<code>{p.targetFile}</code></p>
            <div className="pv-intent-actions">
              <button type="button" className="btn tiny ghost" onClick={() => void copyCode()}><Copy size={12} /> {lang === "zh" ? "复制" : "Copy"}</button>
              <button type="button" className="btn tiny ghost" onClick={() => void writeToFile()}><ExternalLink size={12} /> {lang === "zh" ? "写入新文件" : "Write new file"}</button>
            </div>
          </>
        )}
      </div>
...
function DictPod(props: { onClose: () => void }): React.ReactElement {
  ...
  const rows: Array<[string, number]> = [
    ["L1 " + (lang === "zh" ? "通用编程词典" : "Universal"), Object.keys(L1_UNIVERSAL).length],
    ["L2 " + (lang === "zh" ? "语言语法词典" : "Language"), l2Count],
    ["L3 " + (lang === "zh" ? "框架专属词典" : "Framework"), Object.keys(L3_FRAMEWORK).length],
    ["L4 " + (lang === "zh" ? "项目领域词典" : "Domain"), Object.keys(L4_DOMAIN).length],
    ["L5 " + (lang === "zh" ? "意图变换字典" : "Intent transforms"), 8],
    ["L6 " + (lang === "zh" ? "惯用短语词典" : "Idioms"), Object.keys(L6_IDIOM).length],
    ["L7 " + (lang === "zh" ? "项目模式识别字典" : "Patterns"), Object.keys(L7_PATTERN).length],
    ["L1-L7 " + (lang === "zh" ? "用户补充" : "User learned"), Object.keys(overrides).length],
  ];
  async function exportDict(): Promise<void> {
    ...
    try {
      ...
      pushToast("success", lang === "zh" ? "词典包已导出" : "Dict pack exported", p);
    } catch (e) {
      pushToast("error", lang === "zh" ? "导出失败" : "Export failed", errText(e));
    }
  }
  async function importDict(): Promise<void> {
    ...
    try {
      ...
      pushToast("success", lang === "zh" ? `已导入 ${Object.keys(terms).length} 条词典` : `Imported ${Object.keys(terms).length} terms`);
    } catch (e) {
      pushToast("error", lang === "zh" ? "词典包无效" : "Invalid dict pack", errText(e));
    }
  }
  ...
  return (
    <div className="pv-pod pv-intent-pod">
      <div className="pv-info-head">
        <BookOpen size={13} />
        <strong>{lang === "zh" ? "字典管理" : "Dictionaries"}</strong>
        <button type="button" className="icon-btn tiny pv-close" aria-label="close" onClick={props.onClose}>✕</button>
      </div>
      <div className="pv-info-body">
        <section>
          <h4>{lang === "zh" ? "已加载字典（五级体系，全部离线）" : "Loaded dictionaries"}</h4>
          <ul className="pv-list">
            {rows.map(([name, n]) => <li key={name}>{name} —— {n} {lang === "zh" ? "条" : "entries"}</li>)}
          </ul>
        </section>
        <section>
          <h4>{lang === "zh" ? "用户补充（8.2 教学式反馈）" : "User-learned terms"}</h4>
          {Object.keys(overrides).length === 0 && <p className="dim">{lang === "zh" ? "还没有补充过词条。" : "No learned terms yet."}</p>}
          <ul className="pv-list">
        ...
        <div className="pv-intent-actions">
          <button type="button" className="btn tiny ghost" onClick={() => void importDict()}>{lang === "zh" ? "导入 .dict.json" : "Import .dict.json"}</button>
          <button type="button" className="btn tiny ghost" onClick={() => void exportDict()}>{lang === "zh" ? "导出词典包" : "Export pack"}</button>
        </div>
      </div>
    </div>
  );
