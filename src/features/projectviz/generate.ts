...
export function buildGraph(model: ProjectModel, origin: { x: number; y: number }, lang: Lang): GenGraph {
  ...
  const what = g.node({ ...branchSpecs, html: `<p>❓ <strong>${lang === "zh" ? "是什么" : "What"}</strong></p>`, plain: lang === "zh" ? "是什么" : "What", x: branchX, y: whatY });
  if (what) {
    g.edge(rootNode.key, what.key);
    const cards: Array<{ html: string; plain: string; kind: NodeKind; recordId?: string; w: number; h: number }> = [
      {
        html: `<p>🎯 <strong>${lang === "zh" ? "一句话概括" : "In one sentence"}</strong></p><p style="font-size:12px;opacity:.85">${esc(purposeSentence(detect, scan, lang))}</p>`,
        plain: purposeSentence(detect, scan, lang),
        kind: "info", w: W_CARD, h: H_CARD,
      },
      {
        html: `<p>🧰 <strong>${lang === "zh" ? "技术栈" : "Tech stack"}</strong></p><p style="font-size:12px;opacity:.85">${esc(techStackSentence(detect, lang))}</p>`,
        plain: techStackSentence(detect, lang),
        kind: "info", w: W_CARD, h: H_CARD,
      },
      {
        html: `<p>🚪 <strong>${lang === "zh" ? "怎么启动" : "How it starts"}</strong></p><p style="font-size:12px;opacity:.85">${esc(entryDescription(detect, lang))}</p>`,
  ...
  const has = g.node({ ...branchSpecs, html: `<p>🗂 <strong>${lang === "zh" ? "有什么" : "Has"}</strong></p>`, plain: lang === "zh" ? "有什么" : "Has", x: branchX, y: hasY });
  if (has) {
  ...
  const run = g.node({ ...branchSpecs, html: `<p>⚙️ <strong>${lang === "zh" ? "怎么运行" : "Runs"}</strong></p>`, plain: lang === "zh" ? "怎么运行" : "Runs", x: branchX, y: runY });
  if (run) {
...
export function buildDrillDown(
  ...
  items.forEach((s, i) => {
    ...
    const text = s.kind === "class"
      ? `<p>📐 <strong>${esc(s.name)}</strong><br><span style="opacity:.75;font-size:12px">${lang === "zh" ? "设计图纸（类）" : "Blueprint (class)"} · L${s.line}</span></p>`
      : `<p>⚙️ <strong>${esc(s.name)}()</strong><br><span style="opacity:.75;font-size:12px">${esc(s.params.length > 0 ? s.params.join(", ") : (lang === "zh" ? "无参数" : "no params"))} · L${s.line}</span></p>`;
    nodes.push({
      key,
      html: text,
      plain: s.name,
