...
export function ProjectImportOverlay(props: { state: PvImportState | null; onCancel: () => void }): React.ReactElement | null {
  ...
  const detail = s.error
    ? s.error
    : s.progress?.detail ?? (lang === "zh" ? "准备中…" : "Preparing…");
  return (
    <div className={`pv-progress-strip ${s.error ? "err" : ""}`} role="status" aria-live="polite">
      ...
      <button type="button" className="btn tiny ghost" onClick={props.onCancel}>
        {s.error ? (lang === "zh" ? "关闭" : "Dismiss") : (lang === "zh" ? "后台" : "Bg")}
      </button>
    </div>
  );
}
...
export function FileInfoCard(props: {
  ...
  useEffect(() => {
    ...
    if (!root) {
      setError(lang === "zh" ? "找不到项目根目录（请重新导入项目）" : "Project root not found (re-import the project)");
      return;
    }
    setAnalysis(null);
  ...
  async function teachTerm(term: string): Promise<void> {
    const meaning = await askPrompt({
      title: lang === "zh" ? `“${term}”是什么？` : `What is "${term}"?`,
      initial: "",
    });
    ...
    pushToast("success", lang === "zh" ? "已记住你的解释" : "Learned your explanation");
  }
  ...
  function openSource(): void {
    ...
    void ipc.openPath(joinPath(root, relPath)).catch((e) =>
      pushToast("error", lang === "zh" ? "打开失败" : "Open failed", errMessage(e).message));
  }

  return (
    <div className="pv-info-card card-pop" role="dialog" aria-label={lang === "zh" ? "文件解读" : "File explanation"}>
      ...
      <div className="pv-info-body">
        ...
        {!analysis && !error && <p className="dim">{lang === "zh" ? "正在读取源码…" : "Reading source…"}</p>}
        {analysis && (
          <>
            <section>
              <h4>{lang === "zh" ? "这个文件是干嘛的（五维深度翻译）" : "What this file does (5-dim)"}</h4>
              <p>
                {analysis.role === "doc" || analysis.role === "config" || analysis.role === "asset"
                  ? (lang === "zh" ? "它是一份说明/配置/资源文件。" : "It is a doc/config/asset file.")
              ...
              {preview && (
                <div className="pv-code-preview">
                  ...
                  <div className="dim small">… {lang === "zh" ? "仅预览前 10 行，完整源码走逐行解剖" : "first 10 lines — open anatomy for the rest"}</div>
                </div>
              )}
            </section>
            <section>
              <h4>{lang === "zh" ? "主要功能（大白话）" : "Abilities in plain words"}</h4>
              <ul className="pv-list">
                {(analysis.symbols.length === 0
                  ? [lang === "zh" ? "主要是声明和配置，没有独立的机器或图纸。" : "Mostly declarations; no standalone functions/classes."]
                  : abilityList(analysis, lang, 6, style)
                ).map((line, i) => <li key={i}>{line}</li>)}
              </ul>
            </section>
            <section>
              <h4>{lang === "zh" ? "依赖关系（它借了谁的工具）" : "Dependencies (whose tools it borrows)"}</h4>
              <ul className="pv-list">
                {(analysis.imports.length === 0
                  ? [lang === "zh" ? "不依赖其他模块，自己是独立的。" : "No dependencies."]
            ...
            <section>
              <h4>{lang === "zh" ? "被谁使用" : "Used by"}</h4>
              <p>
                {usedBy === null
                  ? (lang === "zh" ? "（重新导入项目后可计算引用关系）" : "(Re-import the project to compute reverse deps)")
                  : usedBy.length === 0
                    ? (lang === "zh" ? "项目里没有其他文件引用它。" : "Nothing else references it.")
                    : usedBy.slice(0, 6).join(lang === "zh" ? "、" : ", ")}
              </p>
            </section>
            {analysis.binary && (
              <section>
                <h4>{lang === "zh" ? "二进制解剖（可下钻结构）" : "Binary anatomy (drillable)"}</h4>
                <p>
                ...
                <ul className="pv-list">
                  {analysis.binary.sections.slice(0, 12).map((s) => (
                    <li key={s.name}>
                      {lang === "zh" ? "节区" : "Section"} <strong>{s.name}</strong> · {(s.vsize / 1024).toFixed(1)} KB
                      {s.flags.length > 0 ? ` · ${s.flags.join("/")}` : ""}
                    </li>
                  ))}
                  {analysis.binary.imports.slice(0, 8).map((im) => (
                    <li key={im.module}>
                      {lang === "zh" ? "导入" : "Import"} <strong>{im.module}</strong>
            ...
            {analysis.flow && analysis.flow.length > 0 && (
              <section>
                <h4>{lang === "zh" ? "全流程细分（全部分支可无限下钻）" : "Full-flow subdivision (every branch drillable)"}</h4>
      ...
      {analysis && (
        <div className="pv-info-foot">
          <button type="button" className="btn tiny ghost" onClick={openSource}>
            <ExternalLink size={12} /> {lang === "zh" ? "打开源文件" : "Open source"}
          </button>
          <button type="button" className="btn tiny primary" onClick={() => props.onDrill(analysis)}>
            <GitBranch size={12} /> {lang === "zh" ? "下钻进入（函数级）" : "Drill into functions"}
          </button>
          {props.onOpenAnatomy && (
            <button type="button" className="btn tiny ghost" onClick={props.onOpenAnatomy}>
              <FolderOpen size={12} /> {lang === "zh" ? "逐行解剖（完整源码）" : "Line anatomy"}
            </button>
          )}
          {props.onRefToMindmap && (
            ...
            >
              ...
              {props.referenced?.has(relPath)
                ? (lang === "zh" ? "已在导图中" : "Referenced")
                : (lang === "zh" ? "引用到思维导图" : "Ref to mindmap")}
            </button>
          )}
        </div>
      )}
...
export function FlowTreeView(props: { units: import("./types").FlowUnit[]; lang: "zh" | "en" }): React.ReactElement {
  const { lang } = useI18n();
  ...
  if (props.units.length === 0) {
    return <p className="dim small">{lang === "zh" ? "没有识别到分支/循环结构。" : "No branches/loops detected."}</p>;
  }
