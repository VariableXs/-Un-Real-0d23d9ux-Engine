...
export function planIntent(text: string, detect: ProjectDetect, lang: "zh" | "en"): IntentPlan {
  ...
  const unknownTerms = lang === "zh"
    ? ["还没学会这个需求对应的模板"]
    : ["no template learned for this request yet"];
  return {
    matched: false,
    title: lang === "zh" ? "暂时听不懂这个需求" : "Cannot understand this request yet",
    steps: lang === "zh"
      ? [
