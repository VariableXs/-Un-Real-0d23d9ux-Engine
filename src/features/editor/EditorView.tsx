...
export function EditorView(props: { settings: EditorSettingsProps }): React.ReactElement {
  ...
  async function trashCurrent(): Promise<void> {
    ...
    const ok = await askConfirm({
      ...
      body: lang === "zh" ? `将「${doc.title || "无标题"}」移入回收站？` : `Move "${doc.title || "Untitled"}" to trash?`,
      danger: true,
      okLabel: t("delete"),
    });
    if (!ok) return;
  ...
  return await new Promise((resolve) => {
    void import("../../components/ContextMenu").then(({ openContextMenu }) => {
      openContextMenu(window.innerWidth / 2 - 60, 140, [
        ...
        { label: lang === "zh" ? "纯文本 (.txt)" : "Plain text (.txt)", onClick: () => resolve("txt") },
        { label: "JSON (.json)", onClick: () => resolve("json") },
        { label: lang === "zh" ? "取消" : "Cancel", onClick: () => resolve(null) },
      ]);
    });
    ...
    <button
      ...
      onClick={async (e) => {
        ...
        const items: import("../../components/ContextMenu").MenuItem[] =
          folderItems.length > 1 ? folderItems : [{ label: lang === "zh" ? "（暂无文件夹，可在列表中新建）" : "(no folders yet)", disabled: true }];
...
function SlashMenu(props: { editor: Editor | null }): React.ReactElement | null {
  ...
  const items: SlashItem[] = useMemo(
    () => [
      ...
      { label: lang === "zh" ? "无序列表" : "Bullet list", keywords: "list ul 列表", run: (e) => e.chain().focus().toggleBulletList().run() },
      { label: lang === "zh" ? "有序列表" : "Numbered list", keywords: "list ol 编号", run: (e) => e.chain().focus().toggleOrderedList().run() },
      { label: lang === "zh" ? "引用" : "Quote", keywords: "quote 引用", run: (e) => e.chain().focus().toggleBlockquote().run() },
      { label: lang === "zh" ? "分割线" : "Divider", keywords: "hr divider 分割线", run: (e) => e.chain().focus().setHorizontalRule().run() },
      { label: lang === "zh" ? "代码块" : "Code block", keywords: "code 代码", run: (e) => e.chain().focus().setCodeBlock().run() },
    ],
    [lang],
  );
