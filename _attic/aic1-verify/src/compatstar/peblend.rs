...
pub fn run_peblend_checks() -> CheckSet {
    CheckSet::merge(run_peblend_base_checks(), CheckSet::merge(run_peblend_deep_checks(), CheckSet::merge(run_peblend_deep2_checks(), CheckSet::merge(run_peblend_deep3_checks(), CheckSet::merge(run_peblend_deep4_checks(), CheckSet::merge(run_peblend_deep5_checks(), CheckSet::merge(run_peblend_deep6_checks(), CheckSet::merge(run_peblend_deep7_checks(), run_peblend_deep8_checks()))))))))
}
...
const EAT_OFF_NAME_RVA: usize = 16;
...
pub fn run_peblend_deep5_checks() -> CheckSet {
    ...
    m[eat_off + EAT_OFF_NAME_PTRS..eat_off + EAT_OFF_NAME_PTRS + 4].copy_from_slice(&0x320u32.to_le_bytes());
    m[eat_off + EAT_OFF_ORDINALS..eat_off + EAT_OFF_ORDINALS + 4].copy_from_slice(&0x380u32.to_le_bytes());
    // 名称串 @0x340，名指针表 @0x320（指向 0x340），序号 @0x380（序号 0），地址 @0x360。
    m[0x340..0x349].copy_from_slice(b"VarixLoad");
    m[0x320..0x324].copy_from_slice(&0x340u32.to_le_bytes());
...
fn run_peblend_deep8_checks() -> CheckSet {
    ...
    let dirs: alloc::vec::Vec<(u32, u32)> = {
        ...
        v[0] = (0x1000, 0x40); // 导出
        v[1] = (0x2000, 0x80); // 导入
        v[5] = (0xF000, 0x1000); // BASE reloc——0xF000+0x1000 > 0xA000，越界
