...
pub fn run_mlangres_checks() -> CheckSet {
    CheckSet::merge(run_mlangres_base_checks(), CheckSet::merge(run_mlangres_deep_checks(), CheckSet::merge(run_mlangres_deep2_checks(), CheckSet::merge(run_mlangres_deep3_checks(), CheckSet::merge(run_mlangres_deep4_checks(), CheckSet::merge(run_mlangres_deep5_checks(), run_mlangres_deep7_checks()))))))
}
...
pub fn codepage_coverage() -> [(u16, usize); 8] {
    [
        ...
        (CodePage::Cp1251.number(), CP1251_VERIFIED as usize),
        (CodePage::Cp1252.number(), CP1252_VERIFIED as usize),
...
pub const CODEPAGE_REGISTRY2: [CodePageCoverage2; 8] = [
    ...
    CodePageCoverage2 { cp: 932, verified_chars: 97 },   // SJIS 半角片假名 95 + 全角锚点 2
    ...
    CodePageCoverage2 { cp: 437, verified_chars: 128 },  // CP437 全高位（批次六全表）
];
