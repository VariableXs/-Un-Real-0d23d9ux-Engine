...
// ---------------------------------------------------------------------------
// 解码缓冲落位计划（大页池尾部——主册 G-B-14 设计细节）
// ---------------------------------------------------------------------------

/// 解码工作缓冲落位计划。主册 G-B-14 设计细节：「对齐分配走大页池（F051）
/// 尾部」——4MB 大页槽基址天然满足 AVX2 的 64B 行对齐（SIMD 逐行解滤波
/// 不做跨槽拼接）；池尾无空闲槽 → 回退 4KB 常规对齐（graceful 同结果，
/// 主册【状态与异常】回退纪律：AVX2 探测失败无缝回退同纪律的分配面镜像）。
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum BufPlan {
    /// 大页池尾部槽（slot = [`crate::perfstar::bigpage`] 的槽号；
    /// 基址 = 池基址 + slot × 4MB，天然 4MB 对齐 ≥ 64B 行对齐）。
    HugeTail { slot: usize },
    /// 4KB 常规对齐回退（对齐由分配器保证；SIMD 路径不变）。
    Fallback4K,
}

/// 典型单块缓冲需求：流式分块 64 行 × 4K 行宽 = 1MB（单槽 4MB 装得下，
/// 且「缓冲整块落单槽」——跨槽撕裂行连续性，宁回退不大页）。
pub const DECODE_BUF_TYPICAL_BYTES: u64 = STREAM_BLOCK_ROWS as u64 * 4096;

/// 落位决策：需求非零且 ≤ 单槽容量且池尾有槽 → 大页尾部；否则 4K 回退。
/// `tail_free_slot` 来自 [`crate::perfstar::bigpage::HugePagePool::tail_free_slot`]
/// （一处一事实：槽态只此一份，计划器不复制池状态）。
pub fn plan_decode_buf(tail_free_slot: Option<usize>, need_bytes: u64) -> BufPlan {
    match tail_free_slot {
        Some(slot) if need_bytes > 0 && need_bytes <= crate::perfstar::bigpage::HUGE_PAGE_BYTES => {
            BufPlan::HugeTail { slot }
        }
        _ => BufPlan::Fallback4K,
    }
}
...
pub fn run_imgsimd_checks() -> CheckSet {
    ...
    // 8) 解码缓冲落位计划（主册 G-B-14：「对齐分配走大页池（F051）尾部」）：
    //    尾槽在 + 需求 ≤ 单槽 → 大页尾部；无尾槽 / 零需求 / 超单槽 → 4K 回退。
    cs.add(
        "decode_buf_plan",
        plan_decode_buf(Some(15), DECODE_BUF_TYPICAL_BYTES) == BufPlan::HugeTail { slot: 15 }
            && plan_decode_buf(None, DECODE_BUF_TYPICAL_BYTES) == BufPlan::Fallback4K
            && plan_decode_buf(Some(15), 0) == BufPlan::Fallback4K
            && plan_decode_buf(Some(15), (4 << 20) + 1) == BufPlan::Fallback4K,
        "",
    );
    // 9) 池尾联动：F051 池常规落位后尾槽可用，1MB 典型缓冲计划落大页尾部
    //    （跨域对接一处一事实：槽态只读自 bigpage，本域不复制）。
    let mut bp = crate::perfstar::bigpage::HugePagePool::new();
    let _ = bp.reserve_boot(crate::perfstar::bigpage::FB_4K_DUAL_BYTES);
    let tail = bp.tail_free_slot();
    cs.add(
        "decode_buf_pool_tail_live",
...
mod tests {
    ...
    #[test]
    fn decode_buf_plan_edges() {
        // 单槽上界恰好可容纳（4MB = HUGE_PAGE_BYTES）；典型 1MB 缓冲富余。
        assert_eq!(plan_decode_buf(Some(0), 4 << 20), BufPlan::HugeTail { slot: 0 });
        assert_eq!(plan_decode_buf(Some(7), 1), BufPlan::HugeTail { slot: 7 });
        assert_eq!(plan_decode_buf(Some(7), (4 << 20) + 1), BufPlan::Fallback4K);
        assert_eq!(plan_decode_buf(None, 1), BufPlan::Fallback4K);
        // 槽号直通计划（不复制池状态——调用侧从 tail_free_slot 取）。
        let mut bp = crate::perfstar::bigpage::HugePagePool::new();
        assert!(bp.reserve_boot(crate::perfstar::bigpage::FB_4K_DUAL_BYTES));
        let tail = bp.tail_free_slot().unwrap();
        assert_eq!(plan_decode_buf(Some(tail), DECODE_BUF_TYPICAL_BYTES), BufPlan::HugeTail { slot: tail });
    }

    #[test]
    fn decode_buf_typical_fits_stream_block() {
        // 典型缓冲 = 流式单块（64 行 × 4K 行宽）：计划常量与分块规划同源。
        assert_eq!(DECODE_BUF_TYPICAL_BYTES, (STREAM_BLOCK_ROWS * 4096) as u64);
    }
