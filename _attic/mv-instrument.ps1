...
function Get-FileMap {
  ...
  Write-Output ("GF: Base-type=" + $Base.GetType().Name + " Rel-count=" + @($Rel).Count + " Rel0-type=" + @($Rel)[0].GetType().Name)
  $map = @{}
  foreach ($rel in $Rel) {
    $p = Join-Path $Base $rel
    if (-not (Test-Path -LiteralPath $p)) { throw "清单内文件缺失：$rel" }
...
switch ($Action) {
  'Snapshot' {
    ...
    Write-Output "STEP-A"; $existing = @($targets | Where-Object { Write-Output ("  filter: [" + $_ + "]"); Test-Path -LiteralPath (Join-Path $rootPath $_) })
    ...
    Write-Output "STEP-C"; $map = Get-FileMap $rootPath $existing; Write-Output "STEP-D"
    $mf = [pscustomobject]@{
      version = $newV
      label   = $Label
  ...
  'New-Diff' {
    ...
    $hist = @()
    if (Test-Path -LiteralPath $histPath) { $hist = @(Get-Content -LiteralPath $histPath -Raw | ConvertFrom-Json) }
    $prevEntry = $hist | Select-Object -Last 1
    $cur = @{}
