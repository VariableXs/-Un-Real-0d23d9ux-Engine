"""任务24 KV 存储实机驱动（双会话断电恢复）。

会话1（空盘）：KV 账本 format → boot-counter=1 → 写 10 键 → 回读核对 →
        `kvsrv: session=1 ... verdict=ok` → M1 ring3 → 脚本 kill QEMU = 断电。
会话2（同盘面）：boot-counter=2 → 前会话 10 键零丢失核对 →
        `kvsrv: powercut-recovered verdict=ok recovered=10` + `session=2`。
"""
...
INIT_IMG = os.path.join(ATTIC, "kv24-init.img")
SHARED_IMG = os.path.join(ATTIC, "shared-exfat.img")
SERIAL1 = os.path.join(ATTIC, "kv24-session1.log")
SERIAL2 = os.path.join(ATTIC, "kv24-session2.log")
MON_PORT = 14451
...
def launch_qemu(serial_log):
    ...
    return subprocess.Popen(
        [
            ...
            "-device", "nvme,drive=nv2,serial=KV24SHRD",
...
def run_session(tag, serial_log, present, absent, timeout_s):
    ...
    while time.time() < deadline and missing:
        ...
        for ln in text.splitlines():
            if "kvsrv:" in ln and ("failed" in ln or "tainted" in ln):
                power_cut(proc)
                print("[demo] %s FAIL — kvsrv failure line:" % tag, flush=True)
                print("       " + ln, flush=True)
                return None
    ...
    if missing:
        ...
        for m in missing:
            print("       - " + m, flush=True)
        print("[demo] --- kvsrv lines seen ---", flush=True)
        for ln in text.splitlines():
            if "kvsrv:" in ln:
                print("       " + ln, flush=True)
        return None
...
def kvsrv_lines(text):
    return [ln for ln in text.splitlines() if "kvsrv:" in ln]


def main():
    # 全新盘面：init 盘空 512MiB + shared 卷重造。
    ...
    print("[demo] ===== KV POWERCUT DEMO PASS =====", flush=True)
    print("[demo] session-1 kvsrv lines:", flush=True)
    for ln in kvsrv_lines(t1):
        print("       " + ln, flush=True)
    print("[demo] session-2 kvsrv lines:", flush=True)
    for ln in kvsrv_lines(t2):
        print("       " + ln, flush=True)
    return 0
