#!/usr/bin/env python3
"""UEFI VHD 构建提权驱动：跑 uefi-vhd-build.ps1（先补 BOM），轮询至 VHD DONE。"""
import ctypes
import os
import sys
import time
...
SCRIPT = ROOT + r"\_attic\uefi-vhd-build.ps1"
LOG = ROOT + r"\_attic\uefi-vhd-build.log"

# 补 BOM（PS5.1 无 BOM 按 GBK 读，中文字符串乱码炸引号）
raw = open(SCRIPT, "rb").read()
if not raw.startswith(b"\xef\xbb\xbf"):
    open(SCRIPT, "wb").write(b"\xef\xbb\xbf" + raw)
    print("BOM added to uefi-vhd-build.ps1")
